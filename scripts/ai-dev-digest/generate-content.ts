...
// 「1日分の記事を生成する処理」手順4)。TDD対象外(Anthropic APIへのHTTP呼び出しを伴い、プロンプト
...
// 採用された候補1件ずつについてAnthropic Messages APIを呼び出し、日本語の見出し・章立て要約を
// 生成する。プロンプトにはrequirements.md/design.mdの内容をそのまま含める(別ファイルに複製しない。
// content-generation/design.md「設計の前提」参照)。元記事・元動画の内容把握にはAnthropic APIの
// web fetchツールを使い、このスクリプト側では原文を事前取得しない
//
// 実行方法: ANTHROPIC_API_KEY=xxx npx tsx scripts/ai-dev-digest/generate-content.ts <selection.jsonのパス>
...
web_fetchツールで元URLの内容を把握したうえで、次のJSON形式のみを出力してください。前後に説明文・コードブロックの装飾(\`\`\`等)を付けず、JSONオブジェクト単体で応答してください:
...
function extractJson(text: string): GeneratedContent {
  ...
  if (!match) {
    throw new Error(`Anthropic APIの応答からJSONを抽出できませんでした: ${text}`)
  }
  return JSON.parse(match[0]) as GeneratedContent
}

async function generateOne(client: Anthropic, candidate: SelectedTopic, requirements: string, design: string): Promise<GeneratedContent> {
  const response = await client.messages.create({
    ...
    messages: [{ role: 'user', content: buildPrompt(candidate, requirements, design) }],
  })

  const textBlock = [...response.content].reverse().find((block) => block.type === 'text')
  if (!textBlock || textBlock.type !== 'text') {
    throw new Error(`Anthropic APIの応答にテキストブロックが含まれていません(候補: ${candidate.heading})`)
  }
...
async function main() {
  ...
  if (!apiKey) {
    console.error('ANTHROPIC_API_KEY が環境変数に設定されていません(GitHub Actions Secretsの設定。daily-publish/design.md「実行環境の前提」参照)')
    process.exit(1)
  }
  ...
  for (const candidate of selection.topics) {
    ...
    const generated = await generateOne(client, candidate, requirements, design)
    topics.push({
