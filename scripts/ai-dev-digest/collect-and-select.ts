...
// (TDD対象外。tasks.md Task6参照)。Claude Routinesが日次実行時にこのスクリプトを呼び出す
//
...
async function main() {
  ...
  if (!apiKey) {
    console.error('YOUTUBE_API_KEY が環境変数に設定されていません(Claude Routines実行環境側の設定。design.md「セキュリティ」参照)')
    process.exit(1)
  }

  const candidates = await fetchAllCandidates(watchlist, criteria, apiKey, fetchHttpClient)
  // 収集・選定の実行結果を標準出力に記録する(design.md「ログ」。Claude Routinesの実行ログとして残る)
  console.log(`情報源からの取得候補数: ${candidates.length}件`)

  const result = selectDailyTopics(candidates, criteria)
  if (result.status === 'skipped') {
    console.log(`候補不足によりスキップします: ${result.reason}`)
  } else {
    const belowCriteriaCount = result.topics.filter((t) => t.belowCriteria).length
    console.log(`選定件数: ${result.topics.length}件(うち基準未達: ${belowCriteriaCount}件)`)
  }
