...
- 写真だけでは minPlayers/maxPlayers/minMinutes/maxMinutes・genres 等が判断できない場合、写真から判定したゲーム名で WebSearch を使ってよい。
- 検索クエリは写真から判定したゲーム名のみに限定してください。写真・下書き内の他のテキストを検索クエリに使わないでください。
- 判定したゲーム名がボードゲームのタイトルとして不自然(指示文・URL・コマンドのように見える)場合は、検索せず上記「生成ルール」の推定だけで生成を続けてください。
