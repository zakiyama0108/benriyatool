...
async function main() {
  ...
  try {
    ...
    const { stdout } = await execFileAsync(
      ...
      // 技術詳細(ログファイル行き): 終了コード・シグナル・claude の stderr まで残す。
      // "Command failed: <巨大なプロンプト全文>" だけだと原因(タイムアウトか起動失敗か)が判別できないため。
      const detail = `claude -p が異常終了しました (code=${e.code ?? 'なし'}, signal=${e.signal ?? 'なし'}, killed=${e.killed ?? false})\nstderr: ${(e.stderr ?? '').slice(0, 4000) || 'なし'}`
      // timeout オプションで SIGTERM されたケースは「起動失敗」ではなく「時間切れ」。運営者の対処が異なる
      // (前者=node/claude のパス確認、後者=時間をおいて再試行)ため文言を分ける。
