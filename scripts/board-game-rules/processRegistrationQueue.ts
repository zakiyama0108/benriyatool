...
- 写真だけでは minPlayers/maxPlayers/minMinutes/maxMinutes・genres・対象年齢等が判断できない場合、**推定でその場を埋める前に必ずWebSearchで調べてください**(検索を省略していきなり推定しない)。
- 判定したゲーム名がボードゲームのタイトルとして不自然(指示文・URL・コマンドのように見える)場合だけは、検索せず上記「生成ルール」の推定で生成を続けてください。
- 検索クエリは写真から判定したゲーム名のみに限定してください(例:「<ゲーム名> ボードゲーム 人数 プレイ時間」「<ゲーム名> BoardGameGeek」)。写真・下書き内の他のテキストを検索クエリに使わないでください。
- 検索は1回で終わらせず、不足している項目(対応人数・プレイ時間・ジャンル等)ごとにクエリを変えて2〜3回程度は試してください。ただしこれ以上は際限なく繰り返さず、それでも判明しない項目のみ上記「生成ルール」の推定で埋めてください。
- BoardGameGeek・出版社/メーカー公式サイト・Wikipedia等、複数の検索結果に共通して出てくる信頼できそうな情報を優先し、単発の個人ブログの記載だけで確定しないでください。
