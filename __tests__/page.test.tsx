...
describe('【トップページ】ツールカード一覧 - 本番公開済みの全アプリへのリンクを掲載する', () => {
  it('育休給付金シミュレーター(/ikukyu)・資産推移シミュレーター(/life-money-sim)・AI駆動開発ダイジェスト(/ai-dev-digest)・ボドゲのトリセツ(/board-game-rules)・曲名からプレイリスト作成(/spotify-playlist)・週刊トレンド(/trend-digest)・重要ニュースダイジェスト(/news-digest)、すべてのカードが表示されること', () => {
    render(<HubPage />)
