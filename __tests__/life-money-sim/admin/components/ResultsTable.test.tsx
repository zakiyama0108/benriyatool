import { render, screen, within } from '@testing-library/react'
import { describe, it, expect } from 'vitest'
import ResultsTable from '../../../../app/life-money-sim/admin/components/ResultsTable'
import type { AdminRecord } from '../../../../app/life-money-sim/admin/lib/types'

function rec(over: Partial<AdminRecord>): AdminRecord {
  return {
    id: 'x',
    created_at: '2026-07-15T02:30:00.000Z',
    has_spouse: true,
    children_count: 1,
    monthly_salary: 35,
    personal_expense_monthly: 8,
    household_expense_total: 20,
    my_household_share: 10,
    starting_asset: 300,
    investment_mode: true,
    expected_annual_rate: 3,
    event_count: 0,
    final_month_asset: 1000,
    monthly_surplus: 17,
    is_test: false,
    ...over,
  }
}

// 仕様: specs/life-money-sim/admin/requirements.md#一覧表示-1、specs/life-money-sim/admin/requirements.md#一覧表示-3
describe('【管理】保存データの一覧 - 1件ごとの入力・結果を表で表示する', () => {
  it('レコードの各項目が、読める形に整形されて表示されること', () => {
    render(<ResultsTable records={[rec({ has_spouse: true, starting_asset: 300 })]} />)
    const row = screen.getAllByRole('row')[1] // 0行目はヘッダー
    expect(within(row).getByText('あり')).toBeTruthy()
    expect(within(row).getByText('300万円')).toBeTruthy()
    expect(within(row).getByText('資産運用')).toBeTruthy()
  })

  it('貯蓄のみモードで保存されたレコードは、想定利回りの列が空であることが分かる表示になること', () => {
    render(<ResultsTable records={[rec({ investment_mode: false, expected_annual_rate: null })]} />)
    const row = screen.getAllByRole('row')[1]
    expect(within(row).getByText('—')).toBeTruthy()
  })

  it('表示している件数が表示されること', () => {
    render(<ResultsTable records={[rec({ id: 'a' }), rec({ id: 'b' })]} />)
    expect(screen.getByText(/2件/)).toBeTruthy()
  })

  it('対象が0件のとき、データがない旨が表示されること', () => {
    render(<ResultsTable records={[]} />)
    expect(screen.getByText(/0件/)).toBeTruthy()
  })
})
