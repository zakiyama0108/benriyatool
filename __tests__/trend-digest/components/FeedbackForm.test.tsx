...
import FeedbackForm from '../../../app/trend-digest/components/FeedbackForm'

const { saveFeedbackMock } = vi.hoisted(() => ({ saveFeedbackMock: vi.fn() }))
vi.mock('../../../app/trend-digest/lib/saveFeedback', () => ({ saveFeedback: saveFeedbackMock }))

beforeEach(() => {
  saveFeedbackMock.mockReset()
})

// 仕様: specs/trend-digest/article-detail/requirements.md#運営者向けフィードバック-8、specs/trend-digest/article-detail/requirements.md#運営者向けフィードバック-9、specs/trend-digest/article-detail/design.md#フィードバックを送信する処理
describe('フィードバック入力欄 - 自由記述のコメントを入力・送信し、結果を画面に表示する', () => {
  it('入力欄が空文字のとき、送信ボタンが無効化されていること', () => {
    render(<FeedbackForm articleId="2026-09-15-entertainment" topicId="topic-1" />)
