...
import Pagination from '../../../app/trend-digest/components/Pagination'

// 仕様: specs/trend-digest/article-list/design.md#画面設計
describe('ページネーション表示 - 前へ/次へと現在ページ/総ページ数を表示する(1ページのみの場合は非表示)', () => {
  ...
  it('2ページ目の「前へ」は1ページ目(一覧トップ)へのリンクになること', () => {
    ...
    expect(prev.getAttribute('href')).toBe('/trend-digest')
  })

  it('2ページ目の「次へ」は3ページ目へのリンクになること', () => {
    render(<Pagination currentPage={2} totalPages={3} />)
    const next = screen.getByRole('link', { name: '次へ' })
    expect(next.getAttribute('href')).toBe('/trend-digest/page/3')
  })
})
