...
import { paginate } from '../../../app/trend-digest/lib/pagination'

function makeArticles(count: number) {
  return Array.from({ length: count }, (_, i) => ({ id: `article-${i + 1}`, date: `2026-08-${String(i + 1).padStart(2, '0')}` }))
}

// 仕様: specs/trend-digest/article-list/requirements.md#ビジネスルール・制約-2
describe('記事一覧のページ分割 - 新しい順に20件ずつページ分割する', () => {
  it('21件の記事に対し、1ページ目は新しい順の先頭20件になること', () => {
    ...
    expect(result.items[0].id).toBe(articles[0].id)
  })
  ...
  it('21件の記事に対し、2ページ目は残り1件になること', () => {
    ...
    expect(result.items[0].id).toBe(articles[20].id)
  })
