...
import { collectBelowCriteriaRecords, collectFeedback } from '../../../app/news-digest/lib/reviewRecords'

const REVIEW_FIXTURES_DIR = path.join(__dirname, '../fixtures/articles-review')

// 仕様: specs/news-digest/monthly-review/requirements.md#見直しの実行-1、specs/news-digest/content-selection/requirements.md#採用基準(カテゴリごとの定量判定)-5
describe('基準未達記録の集計 - 直近1ヶ月分の記事データからbelowCriteria:trueのトピックのみを抽出する', () => {
  it('集計期間内(sinceDate以降)の基準未達トピックのみが抽出されること(期間外のファイルは除外される)', () => {
    ...
    expect(records[0]).toMatchObject({
      ...
      category: 'kanagawa',
      sourceName: '神奈川新聞',
      belowCriteriaReason: '専用枠での優先掲載(全国基準未達)',
    })
  })
...
// 仕様: specs/news-digest/monthly-review/design.md#見直しの材料を集める処理
describe('フィードバックの取得 - 集計期間・is_test=falseの条件でnews_digest_feedbackをSELECTする', () => {
