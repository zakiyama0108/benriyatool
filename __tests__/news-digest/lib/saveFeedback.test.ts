...
import { isTestData, saveFeedback } from '../../../app/news-digest/lib/saveFeedback'
...
afterEach(() => {
  vi.unstubAllEnvs()
  window.history.replaceState(null, '', '/news-digest/')
})

// 仕様: specs/news-digest/article-detail/requirements.md#フィードバックの保存・権限-3
describe('テストデータの判定 - 開発環境またはURLのtest=1パラメータで動作確認用の投稿かを判定する(life-money-sim/lib/saveResult.tsと同一ロジック)', () => {
  ...
  it('本番ビルドでもURLにtest=1パラメータを付けて開いている場合、テストデータと判定されること', () => {
    ...
    window.history.replaceState(null, '', '/news-digest/2026-09-09/?test=1')
    expect(isTestData()).toBe(true)
  })
...
// 仕様: specs/news-digest/article-detail/requirements.md#運営者向けフィードバック-10、specs/news-digest/article-detail/requirements.md#フィードバックの保存・権限-3
describe('フィードバックの保存 - 記事日付・トピックID・入力内容をnews_digest_feedbackテーブルへinsertする', () => {
  it('article_date・topic_id・comment・is_testが正しいカラム名でinsertされ、保存成功時にtrueが返ること', async () => {
    vi.stubEnv('NODE_ENV', 'production')
    await expect(
      saveFeedback({ articleDate: '2026-09-09', topicId: 'topic-1', comment: 'この選定はもう不要かもしれません' })
    ).resolves.toBe(true)

    expect(fromMock).toHaveBeenCalledWith('news_digest_feedback')
  ...
  it('Supabaseが{error}を返した場合(RLS拒否等)、falseが返ること', async () => {
    ...
    await expect(saveFeedback({ articleDate: '2026-09-09', topicId: 'topic-1', comment: 'コメント' })).resolves.toBe(
      false
    )
  })
