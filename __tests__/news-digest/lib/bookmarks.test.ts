...
import {
  ...
  updateBookmark,
  deleteBookmark,
} from '../../../app/news-digest/lib/bookmarks'
...
// 仕様: specs/news-digest/bookmark/requirements.md#トピックへの付箋-1、specs/news-digest/bookmark/requirements.md#表示範囲・権限-1
describe('記事内の自分の付箋の一覧取得 - 指定した記事日付の自分の付箋をトピックIDをキーにしたMapで取得する', () => {
  it('取得した付箋がトピックIDをキーにしたMap(id・memoを持つ)へ変換されること', async () => {
    ...
    const result = await fetchBookmarksByArticleDate('2026-09-09')
    expect(fromMock).toHaveBeenCalledWith('news_digest_bookmarks')
    expect(eqSelectMock).toHaveBeenCalledWith('article_date', '2026-09-09')
  ...
  it('取得に失敗した場合、呼び出し元が「すべて未付箋」として扱えるよう例外を投げること', async () => {
    ...
    await expect(fetchBookmarksByArticleDate('2026-09-09')).rejects.toThrow()
  })
})

// 仕様: specs/news-digest/bookmark/requirements.md#付箋した記事一覧-14
describe('付箋一覧の取得 - ログイン中の本人の付箋を更新日時の新しい順ですべて取得する', () => {
  it('取得した付箋がarticleDate・topicId・memoを持つ配列に変換され、updated_atの新しい順で問い合わせること', async () => {
    orderMock.mockResolvedValue({
      data: [{ id: 'a', article_date: '2026-09-09', topic_id: 'topic-1', memo: '気になる' }],
      error: null,
    })
    const result = await fetchAllBookmarks()
    expect(fromMock).toHaveBeenCalledWith('news_digest_bookmarks')
    expect(orderMock).toHaveBeenCalledWith('updated_at', { ascending: false })
    expect(result).toEqual([{ id: 'a', articleDate: '2026-09-09', topicId: 'topic-1', memo: '気になる' }])
  })
...
// 仕様: specs/news-digest/bookmark/requirements.md#トピックへの付箋-1、specs/news-digest/bookmark/requirements.md#トピックへの付箋-4、specs/news-digest/bookmark/requirements.md#文字数・件数の制約-4
describe('新規に付箋を貼る - ログイン中の本人のuser_idで記事日付・トピックID・メモを新規保存する', () => {
  it('保存に成功した場合、ログイン中の本人のuser_idで正しいカラム名でINSERTされ、新規行のidが返ること', async () => {
    const id = await createBookmark({ articleDate: '2026-09-09', topicId: 'topic-1', memo: '気になる' })
  ...
  it('Supabaseが{error}を返した場合、nullが返ること', async () => {
    ...
    await expect(createBookmark({ articleDate: '2026-09-09', topicId: 'topic-1', memo: '気になる' })).resolves.toBeNull()
  })
...
// 仕様: specs/news-digest/bookmark/requirements.md#付箋の編集・削除-7
...
// 仕様: specs/news-digest/bookmark/requirements.md#付箋の編集・削除-8
