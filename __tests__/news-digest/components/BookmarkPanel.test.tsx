...
import BookmarkPanel from '../../../app/news-digest/components/BookmarkPanel'
import { createBookmark, updateBookmark, deleteBookmark } from '../../../app/news-digest/lib/bookmarks'
...
// 仕様: specs/news-digest/bookmark/requirements.md#トピックへの付箋-1、specs/news-digest/bookmark/requirements.md#トピックへの付箋-2、specs/news-digest/bookmark/requirements.md#トピックへの付箋-3、specs/news-digest/bookmark/requirements.md#トピックへの付箋-6、specs/news-digest/bookmark/requirements.md#文字数・件数の制約-3
describe('付箋の新規作成 - 未付箋のトピックで「付箋を貼る」操作からその場でメモを保存する', () => {
  it('initialBookmarkがnullの場合、「付箋を貼る」操作のみが表示されること', () => {
    render(<BookmarkPanel articleDate="2026-09-09" topicId="topic-1" initialBookmark={null} />)
  ...
  it('入力欄で「保存」を押すと、articleDate・topicId・トリムしたメモでcreateBookmarkが呼ばれ、成功時は入力欄が閉じ保存内容が表示されること', async () => {
    createBookmarkMock.mockResolvedValue('bookmark-1')
    const onChange = vi.fn()
    render(<BookmarkPanel articleDate="2026-09-09" topicId="topic-1" initialBookmark={null} onChange={onChange} />)
    fireEvent.click(screen.getByRole('button', { name: '付箋を貼る' }))
    fireEvent.change(screen.getByRole('textbox'), { target: { value: '  あとで読み返す  ' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))

    await waitFor(() => expect(screen.queryByRole('textbox')).toBeNull())
    expect(createBookmarkMock).toHaveBeenCalledWith({ articleDate: '2026-09-09', topicId: 'topic-1', memo: 'あとで読み返す' })
...
// 仕様: specs/news-digest/bookmark/requirements.md#トピックへの付箋-4、specs/news-digest/bookmark/requirements.md#付箋の編集・削除-7、specs/news-digest/bookmark/requirements.md#付箋の編集・削除-8、specs/news-digest/bookmark/requirements.md#付箋の編集・削除-9
describe('付箋の編集・削除 - 既に付箋を貼ったトピックで内容を書き直す、または削除する', () => {
  it('initialBookmarkがある場合、保存済みのメモ内容と「編集」「削除」操作が表示されること', () => {
    render(
      <BookmarkPanel articleDate="2026-09-09" topicId="topic-1" initialBookmark={{ id: 'bookmark-1', memo: '気になる' }} />
    )
