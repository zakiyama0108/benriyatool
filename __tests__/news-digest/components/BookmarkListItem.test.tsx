...
import BookmarkListItem from '../../../app/news-digest/components/BookmarkListItem'
import { updateBookmark, deleteBookmark } from '../../../app/news-digest/lib/bookmarks'
...
// 仕様: specs/news-digest/bookmark/requirements.md#付箋した記事一覧-11、specs/news-digest/bookmark/requirements.md#付箋した記事一覧-12
describe('付箋一覧の1項目 - トピック見出し・付箋メモを表示し、対象トピックへ遷移できる', () => {
  it('トピック見出し・付箋メモの内容が表示されること', () => {
    render(
      <BookmarkListItem
        articleDate="2026-09-09"
        topicHeading="保育料の一部が来年度から無償化される"
  ...
  it('トピック見出しのリンクが、対象記事の該当トピックへのアンカー(/news-digest/<date>#<topicId>)であること', () => {
    render(
      <BookmarkListItem
        articleDate="2026-09-09"
        topicHeading="保育料の一部が来年度から無償化される"
    ...
    const link = screen.getByRole('link', { name: '保育料の一部が来年度から無償化される' })
    expect(link.getAttribute('href')).toBe('/news-digest/2026-09-09#topic-1')
  })
})

// 仕様: specs/news-digest/bookmark/requirements.md#付箋した記事一覧-13
