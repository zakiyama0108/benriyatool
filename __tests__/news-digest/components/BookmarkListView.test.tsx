...
import BookmarkListView from '../../../app/news-digest/components/BookmarkListView'
import { getSession, onAuthChange } from '../../../app/lib/adminAuth'
import { fetchAllBookmarks } from '../../../app/news-digest/lib/bookmarks'
...
const topicIndex = {
  '2026-09-09:topic-1': '保育料の一部が来年度から無償化される',
  '2026-09-02:topic-2': '県内の交通規制が変更される',
}
...
// 仕様: specs/news-digest/bookmark/requirements.md#付箋した記事一覧-10
...
// 仕様: specs/news-digest/bookmark/requirements.md#付箋した記事一覧-10、specs/news-digest/bookmark/requirements.md#付箋した記事一覧-14
describe('付箋一覧ページ - ログイン中は自分の付箋一覧を表示する', () => {
  ...
  it('1件以上の場合、updated_atの新しい順(fetchAllBookmarksが返した順)でBookmarkListItemが並ぶこと', async () => {
    getSessionMock.mockResolvedValue(makeSession('reader@example.com'))
    fetchAllBookmarksMock.mockResolvedValue([
      { id: 'a', articleDate: '2026-09-09', topicId: 'topic-1', memo: '新しい付箋' },
      { id: 'b', articleDate: '2026-09-02', topicId: 'topic-2', memo: '古い付箋' },
    ])
    render(<BookmarkListView topicIndex={topicIndex} />)
  ...
  it('topicIndexに対応するキーが無い付箋は一覧から除外されること(存在しないリンク先を作らないため)', async () => {
    getSessionMock.mockResolvedValue(makeSession('reader@example.com'))
    fetchAllBookmarksMock.mockResolvedValue([
      { id: 'a', articleDate: '2026-09-09', topicId: 'topic-1', memo: '存在するトピックの付箋' },
      { id: 'b', articleDate: '2026-09-09', topicId: 'topic-999', memo: '存在しないトピックの付箋' },
    ])
    render(<BookmarkListView topicIndex={topicIndex} />)
...
// 仕様: specs/news-digest/bookmark/design.md「付箋一覧を取得して表示する処理」
