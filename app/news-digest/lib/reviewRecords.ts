...
// 月次見直しの材料集め(仕様: monthly-review/requirements.md#見直しの実行-1、
// design.md「見直しの材料を集める処理」)。
// collectFeedbackはSQL実行を注入可能にしてpg依存を持たない設計にしている
// (ai-dev-digestのapp/ai-dev-digest/lib/reviewRecords.tsと同じ設計判断。fetchCandidates.tsの
// HttpClientとも同じパターン)。design.mdは本ロジックを
// scripts/news-digest/collect-review-data/collectReviewData.tsに置く想定だが、
// そのディレクトリは独立した依存関係(pg/dotenv)を持つ隔離パッケージのため本体の
// tsconfigプロジェクトに含まれない(tsconfig.json参照)。pg非依存のこのロジック自体は
// 本体のvitestで完全にテストできるため、テスト可能性を保つ目的でapp/news-digest/lib/に置き、
...
// 実際のpg接続はscripts/news-digest/collect-review-data/側の薄いCLIが担う
export type FeedbackQueryExecutor = (sql: string, params: unknown[]) => Promise<FeedbackRow[]>

// news_digest_feedbackから、sinceDate以降・is_test=falseのレコードを取得する
// (design.md「見直しの材料を集める処理」手順2。領域で絞り込まず全件取得する)
export async function collectFeedback(sinceDate: string, executeQuery: FeedbackQueryExecutor): Promise<FeedbackRecord[]> {
  const rows = await executeQuery(
    'select article_date, topic_id, comment from news_digest_feedback where article_date >= $1 and is_test = false order by article_date desc',
    [sinceDate]
  )
