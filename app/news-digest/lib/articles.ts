...
// content/news-digest/articles/ を既定の格納場所とする(design.md「前提: 記事データの形式」)。
// __tests__からはフィクスチャ用ディレクトリを明示的に渡してテストする(Task4)
const DEFAULT_ARTICLES_DIR = path.join(process.cwd(), 'content/news-digest/articles')

// 指定ディレクトリ配下の記事データをすべて読み込み、日付の新しい順に返す。
// ディレクトリ自体が存在しない場合は「運用開始直後で記事が1件もない」状態として
// 空配列を返す(例外にしない)。JSONのパース・スキーマ違反は例外として呼び出し元に伝播させ、
// 壊れたデータのままビルドが通ってしまう事故を防ぐ(design.md#エラーハンドリング)
