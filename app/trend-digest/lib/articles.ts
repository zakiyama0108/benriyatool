...
// content/trend-digest/articles/ を既定の格納場所とする(design.md「前提: 記事データの形式」)。
// __tests__からはフィクスチャ用ディレクトリを明示的に渡してテストする(Task4)
const DEFAULT_ARTICLES_DIR = path.join(process.cwd(), 'content/trend-digest/articles')
...
// 指定したIDの記事を1件返す。該当ファイルが存在しないIDは「記事なし」を表す
// nullを返す(存在しないIDへのアクセスは通常のnot found扱いであり、例外にしない)
