...
export default function FeedbackForm({ articleId, topicId }: Props) {
  ...
  async function handleSubmit() {
    setStatus('sending')
    const succeeded = await saveFeedback({ articleId, topicId, comment: comment.trim() })
    if (succeeded) {
      setComment('')
      setStatus('sent')
    } else {
  ...
  return (
    <div className="mt-3 space-y-2">
      <textarea
        ...
        className="w-full rounded-xl border border-gray-200 p-2.5 text-sm outline-none focus:border-amber-400"
        rows={2}
      />
      <div className="flex items-center gap-3">
        <button
          ...
          disabled={!isSendable}
          className="rounded-full bg-amber-500 px-4 py-1.5 text-xs font-bold text-white shadow-sm disabled:opacity-50"
        >
          送信
        </button>
        {status === 'sent' && <span className="text-xs text-amber-600">送信しました</span>}
