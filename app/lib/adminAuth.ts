...
// 管理画面共通の認証ロジック(ログイン開始・ログアウト・セッション取得・閲覧権限の確認)。
// admin_emails許可リストは全アプリ共用のため、テーブル名に依存しないこのロジックも
// アプリ間で共通化する(仕様: specs/life-money-sim/admin/tasks.md T1、ikukyu/adminと共用)
