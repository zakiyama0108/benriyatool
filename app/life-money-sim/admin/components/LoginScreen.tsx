...
// (仕様: requirements.md#ログイン・アクセス制御-1、requirements.md#ログイン・アクセス制御-3。ikukyu/admin/components/LoginScreen.tsxと同等のロジック)
export default function LoginScreen({ variant, onLogin, onLogout }: Props) {
  return (
    <div className="mx-auto max-w-sm px-4 py-16 text-center space-y-4">
      ...
      {variant === 'login' ? (
        <>
          <p className="text-sm text-teal-700/70">閲覧するにはログインが必要です。</p>
          <button
            onClick={onLogin}
            className="w-full rounded-full bg-teal-600 py-2.5 text-sm font-medium text-white"
          >
            Googleでログイン
          </button>
        </>
        ...
        <>
          ...
          <button
            onClick={onLogout}
            className="w-full rounded-full border border-gray-300 py-2.5 text-sm font-medium text-gray-700"
          >
            ログアウト
          </button>
        </>
