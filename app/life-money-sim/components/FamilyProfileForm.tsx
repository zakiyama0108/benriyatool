...
export default function FamilyProfileForm({ profile, onChange, hasSpouse, onHasSpouseChange }: Props) {
  ...
  return (
    <div className="space-y-3 rounded-[18px] bg-[#E1F2EF] p-5 shadow-[0_10px_24px_-16px_rgba(20,158,146,0.35)]">
      <p className="text-sm font-bold text-[#1C2A28]">家族構成</p>
      ...
      <label className="block text-xs text-[#7E9491]">
        ...
        <input
          ...
          className="mt-1 w-full rounded-full border border-[#DCEFEC] bg-white px-4 py-2 text-sm outline-none focus:border-[#149E92]"
        />
      </label>

      <label className="flex items-center gap-2 text-xs text-[#7E9491]">
      ...
      {profile.childrenBirthMonths.map((birthMonth, i) => (
        <label key={i} className="block text-xs text-[#7E9491]">
          {`子ども${i + 1}の生年月(任意)`}
          <input
            type="month"
            value={birthMonth ?? ''}
