...
export default function EventListInput({ bonuses, onBonusesChange, events, onEventsChange }: Props) {
  ...
  return (
    <div className="space-y-4 rounded-[18px] bg-[#E1F2EF] p-5 shadow-[0_10px_24px_-16px_rgba(20,158,146,0.35)]">
      <div className="space-y-2">
        <p className="text-sm font-bold text-[#8A5E0E]">賞与(通常のボーナスとは別に登録)</p>
          ...
          {bonuses.map((bonus, i) => (
            <li key={i} className="flex items-center gap-2">
              <input
                ...
                className="rounded-full border border-[#F2B84B]/50 bg-white px-3 py-1.5 text-xs outline-none"
              />
              <input
                type="number"
                aria-label="賞与の金額"
                ...
                placeholder="万円"
                className="w-24 rounded-full border border-[#F2B84B]/50 bg-white px-3 py-1.5 text-xs tabular-nums outline-none"
              />
              <button
                type="button"
        ...
        <button
          type="button"
          onClick={addBonus}
          className="rounded-full bg-[#FDECC9] px-4 py-1.5 text-xs font-medium text-[#8A5E0E] hover:bg-[#F2B84B]/40"
        >
          + 賞与を追加
        </button>
      </div>
      ...
      <div className="space-y-2">
        <p className="text-sm font-bold text-[#FF6F59]">イベント(一時的な追加支出)</p>
        <ul className="space-y-2">
          {events.map((event, i) => (
            <li key={i} className="flex flex-wrap items-center gap-2">
              <input
                ...
                className="rounded-full border border-[#FF6F59]/40 bg-white px-3 py-1.5 text-xs outline-none"
              />
              <input
                type="text"
                aria-label="イベントの名目"
                ...
                placeholder="名目(例: 結婚)"
                className="min-w-0 flex-1 rounded-full border border-[#FF6F59]/40 bg-white px-3 py-1.5 text-xs outline-none"
              />
              <input
                type="number"
                aria-label="イベントの金額"
                ...
                placeholder="万円"
                className="w-24 rounded-full border border-[#FF6F59]/40 bg-white px-3 py-1.5 text-xs tabular-nums outline-none"
              />
              <button
                type="button"
        ...
        <button
          type="button"
          onClick={addEvent}
          className="rounded-full bg-[#FFE4DD] px-4 py-1.5 text-xs font-medium text-[#FF6F59] hover:bg-[#FF6F59]/25"
        >
          + イベントを追加
        </button>
      </div>
