...
// 未ログイン時に「この試算を実行する」ボタンの近くへ表示するマイシナリオへのログイン誘導案内。
...
// design.md#マイシナリオ操作の表示を出し分ける処理。ボタン名の参照は save-result/requirements.md#機能要件-3
// の文言変更にあわせて更新)
export default function ScenarioLoginPrompt({ onLoginClick }: Props) {
  return (
    <div className="space-y-2 rounded-[35px] border border-lms-line-strong bg-lms-card p-5 text-sm text-lms-ink/70">
      <p>
        ログインすると、賞与・イベントの名目や生年月も含めた入力項目一式を「マイシナリオ」として保存・復元できます(この試算を実行するでは匿名分析のため個人情報は保存されません)。
      </p>
      <button
