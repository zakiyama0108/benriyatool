...
type Props = {
  // 管理画面を表示している間だけtrueを渡す(BoardGameNavのactiveが"admin"かどうか)。
  // 未指定(BoardGameNav側でadmin以外がactiveのとき)はfalse相当
  active?: boolean
}

// 共通ナビ(左サイドバー)に置く、運営者専用の管理画面導線(仕様: admin/requirements.md#ログイン・アクセス制御-5、
// admin/requirements.md#画面レイアウト・回遊導線-13、admin/design.md「共通ナビに管理画面への導線を表示する処理」)。
// これは運営者が管理画面へ素早く到達するための利便であって、アクセス制御ではない
// (実際の保護はRLS/Storageポリシーが担う)。判定ロジックは既存の共通運営者判定(isAuthorizedAdmin)を
// そのまま再利用し、新規ロジックは持たない。管理画面自身も他画面と同じBoardGameNavを表示するため、
// activeがtrue(admin画面を表示中)の場合は他ナビ項目と同じ現在地体裁(aria-current="page")で描画する
export default function AdminNavLink({ active = false }: Props) {
