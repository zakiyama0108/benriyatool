    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-3xl px-6 py-12 pb-20 space-y-8">
        ...
        <section id="terms" className="rounded-2xl border border-gray-200 bg-white p-10">
          ...
          <p className="body-text mt-3">
            本サービスの一部機能(ボードゲームのルール確認)では、利用者から寄せられた依頼をもとに、運営者が独自の言い回しで再構成したルール解説を掲載している(説明書原文の転載ではない)。また、ゲームを紹介する画像は、投稿者または運営者が実物を撮影したもの、もしくはそれを基にAIで加工したものを掲載しており、第三者の画像をそのまま転載するものではない。著作権者から掲載内容の修正・削除の要望があれば、問い合わせ先まで連絡があり次第、速やかに対応する。
          </p>
          <p className="body-text mt-3">
        ...
        <section id="privacy" className="rounded-2xl border border-gray-200 bg-white p-10">
          ...
          <p className="mt-1 mb-8 text-sm text-gray-400">最終更新日：2026年9月13日</p>
          ...
          <p className="body-text mt-3">
            資産推移シミュレーターでは、任意でGoogleアカウントによるログインを利用できます。ログインした場合、Googleアカウントのメールアドレスを取得し、利用者ご自身が「マイシナリオ」として保存を選択した入力内容（収入・支出の内訳、家族の生年月、開始資産額、賞与・イベントの登録内容など）を、ログインしたアカウントに紐付けて保存します。ログインは任意であり、ログインしない場合はこれらの情報を保存しません。
          </p>
          <p className="body-text mt-3">
            AI駆動開発ダイジェストでは、任意でGoogleアカウントによるログインを利用できます。ログインした場合、Googleアカウントのメールアドレスを取得し、利用者ご自身が「付箋」機能で入力したメモ内容を、対象のトピック・ログインしたアカウントに紐付けて保存します。ログインは任意であり、ログインしない場合はこれらの情報を保存しません。
          </p>
          <p className="body-text mt-3">
            ボードゲームのルール確認では、任意でGoogleアカウントによるログインを利用できます。ログインした場合、Googleアカウントのメールアドレスと表示名(氏名を含む場合があります)を取得します。利用者ご自身が登録した「お気に入り」のゲームは、ログインしたアカウントに紐付けて保存し、保存したご本人のみが閲覧できます。また、利用者が投稿したコメントは、表示名(氏名を含む場合があります)と本文を、対象ゲームのページに<strong>公開</strong>して保存します。コメントの表示名・本文は、ログインの有無にかかわらず誰でも閲覧できるため、公開したくない情報を含めないようご注意ください。ログインは任意で、ログインしない場合はこれらの情報を保存しません(コメントの投稿・お気に入りの登録にはログインが必要です)。
          </p>
          <p className="body-text mt-3">
            重要ニュースダイジェストでは、任意でGoogleアカウントによるログインを利用できます。ログインした場合、Googleアカウントのメールアドレスを取得し、利用者ご自身が「付箋」機能で入力したメモ内容を、対象のトピック・ログインしたアカウントに紐付けて保存します。ログインは任意であり、ログインしない場合はこれらの情報を保存しません。
          </p>
          <p className="body-text mt-3">
            曲名からプレイリスト作成では、機能の利用にSpotifyアカウントによるログイン(認可)が必要です。ログインすると、Spotifyの表示名・プロフィール画像・ユーザーIDを取得します。これらは画面表示のためにご利用中のブラウザ上でのみ一時的に保持し、運営者のサーバーやデータベースには送信・保存しません。作成したプレイリストや入力した曲名も運営者側には保存しません。
          </p>
          ...
          <ul className="mt-2 mb-3 list-disc pl-5 space-y-1.5 text-sm text-gray-600">
            ...
            <li>資産推移シミュレーターのマイシナリオ機能で、ログインした利用者ご本人に対して保存内容を復元するため</li>
            <li>AI駆動開発ダイジェストの付箋機能で、ログインした利用者ご本人に対して保存内容を復元するため</li>
            <li>重要ニュースダイジェストの付箋機能で、ログインした利用者ご本人に対して保存内容を復元するため</li>
            <li>ボードゲームのルール確認で、お気に入りをご本人に復元し、コメントを利用者同士の助け合い(ルールの補足・誤りの指摘など)のために公開表示するため</li>
            <li>曲名からプレイリスト作成で、取得した情報を利用者ご自身のSpotifyアカウントへの新規プレイリスト作成にのみ使用するため</li>
          </ul>
          <p className="body-text">統計データは匿名の集計値として扱い、個人の行動追跡には使用しません。マイシナリオ・付箋・お気に入りとして保存された内容は、保存した本人のみが閲覧・編集・削除でき、他の利用者や運営者が閲覧することはありません(コメントは公開表示され、投稿者本人と運営者が削除できます)。</p>
