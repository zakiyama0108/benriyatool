...
- Task 1: 代表見出しの選択(仕様: requirements.md#配信内容-3〜5、design.md「代表見出しを選ぶ処理」)
  - 🔴 `selectRepresentatives(article)`について次を確認するテストを書く: ジャンルごとに影響度の最も大きい予測が選ばれる/同じ影響度が2本なら時間軸の近い方が選ばれる/性・恋愛ジャンルは含まれない/掲載した予測が1本もないジャンル(両枠が候補なし・収集失敗・生成失敗のいずれかの組み合わせ)は含まれない/影響度の大きい順、同じ影響度ではジャンル順に並ぶ
