...
## 1. 概要
10ジャンルについて、公開されている未来予測・考察記事を近未来(1〜5年後)・中期未来(6〜20年後)・長期未来(20〜50年後)・超長期未来(50年後〜)の時間軸ごとに選び、影響度(大・中・小)付きで要約して毎週木曜に公開するアプリ。 URL: `/future-digest`
...
- 4つの時間軸を配信回ごとに2つずつ交互に扱い(奇数回=近未来+長期未来、偶数回=中期未来+超長期未来)、1回あたり最大20本に抑える
...
| spec | 役割 | 依存 | 状態 |
|---|---|---|---|
| [content-selection](content-selection/requirements.md) | 10ジャンル×その回の2時間軸の枠ごとに、影響度が最も大きく未配信の未来予測記事を1本選ぶ | weekly-publishの実行タイミングに従う | 仕様のみ(未実装) |
...
| [weekly-publish](weekly-publish/requirements.md) | 毎週木曜の収集・選定・要約・公開を自動で行い、完全自動マージする | content-selection・content-generationの結果を公開する | 仕様のみ(未実装) |
