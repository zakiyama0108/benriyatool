...
- [4] その回の実行を公開・スキップ・失敗のいずれとして扱うかは[weekly-publish/requirements.md#掲載件数の保証](../weekly-publish/requirements.md)の判定に従う(候補なしの枠だけで採用0件だった場合に限り正常なスキップとし、採用0件で収集失敗の枠が1つ以上ある場合は実行を失敗とする)【推測】
