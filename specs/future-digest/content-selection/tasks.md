...
- Task 6: ジャンルごとの収集CLI(仕様: design.md「ジャンルごとに候補を集める処理」「エラーハンドリング」)
  ...
  - 🟢 `scripts/future-digest/collect-candidates.ts`を実装する。プロンプトにはジャンルの説明(注目テーマ)、時間軸の定義、requirements.md#採用基準・#影響度・#配信済みの記事・予測の除外の内容、配信済みの一覧、応答JSONの形、性・恋愛ジャンルの収集時の制約を含める。許可ツールはWebSearch・WebFetchに限る
