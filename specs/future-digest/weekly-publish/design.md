...
- 手順:
  ...
  2. [content-selection](../content-selection/design.md)の収集・選定を実行し、今回の回数・採用した候補・候補なしの枠を受け取る
  3. 全枠で候補がなかった場合は、記事を作らず後述「公開をスキップする処理」に進む(requirements.md#掲載件数の保証-1)
  4. 採用した候補ごとに[content-generation](../content-generation/design.md)の生成を行う。1本の生成が一時的な失敗に終わった場合は、同じ候補を最大2回まで(初回+1回)起動し直し、それでも失敗した候補は「生成に失敗した枠」として除き、次の候補に進む(requirements.md#掲載件数の保証-2)
  5. 生成に1本以上成功した場合は、記事データ(回数・発行日・予測・掲載できなかった枠)を組み立てる。掲載できなかった枠には、候補なしの枠と生成に失敗した枠の両方を理由つきで入れ、その回に有効な全ジャンル×2時間軸の枠が過不足なく記事に現れるようにする。組み立てた記事を`content/future-digest/articles/<実行日>.json`に書き出す
...
sequenceDiagram
    ...
    select -->> actions: 回数・採用した候補・候補なしの枠
    alt 全枠で候補なし
...
- 対象: 上記で作ったブランチ
- 手順:
  1. `main`向けにPRを作る(タイトル例:「[future-digest] 2026-10-01を公開」。本文に回数・時間軸・採用件数・候補なしの枠の数を書く)
...
- 手順:
  ...
  2. 自動マージの対象はこのブランチパターンに限る。[source-review](../source-review/design.md)のPR(`future-digest/source-review/**`)は対象外で、運営者の承認を必須とする(requirements.md#自動マージの範囲-3)
...
- 実行ごとに、実行日・回数・時間軸2区分・採用件数・候補なしの枠の数・生成に失敗した枠の数・PRのURL・結果(公開/スキップ/失敗)をワークフローの実行ログに出す
