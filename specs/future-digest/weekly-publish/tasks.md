...
- Task 3: ワークフロー本体(仕様: design.md「実行環境の前提」「1回分の記事を生成する処理」「PRを作成しCIの結果を待つ処理」「PRを自動マージする処理」「CI失敗時に記録する処理」「公開をスキップする処理」)(TDD対象外。GitHub Actionsの定義のため。trend-digest-weekly.ymlと同じ構造で実装する)
  ...
  - `publish`ジョブ: `FUTURE_DIGEST_GH_PAT`でcheckout→Claude Code CLIのインストール→実行日(JST)の算出→ブランチ作成→`collect-and-select.ts`→採用0件で空いた枠がすべて候補なしならスキップ(成功で終了)/収集失敗が1つ以上混在すれば失敗(非ゼロ終了)→`generate-content.ts`→`write-article.ts`→コミット・push・PR作成・`gh pr merge --auto --squash`
