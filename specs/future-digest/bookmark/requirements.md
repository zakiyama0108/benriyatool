...
### 記事への付箋
- [1] ログイン中の読者は、記事詳細ページに表示される予測ごとに、メモ付きの付箋を貼れる
- [2] メモが空、または空白文字だけの場合は保存できない
- [3] メモは200文字までとする
- [4] 1件の予測に1人の読者が貼れる付箋は1件までとする。付箋済みの予測への操作は、既存の付箋の編集として扱う
- [5] 未ログインの訪問者には、付箋を貼る操作(ボタン等)を表示しない
- [6] 付箋を貼る・編集する操作は、その予測の位置でその場に開く形で行い、別画面・モーダルには移らない

### 付箋の編集・削除
- [7] 付箋済みの予測では、メモを書き直して上書き保存できる
- [8] 付箋済みの予測では、付箋を削除できる。削除すると、未付箋の状態に戻る

### 付箋の一覧
- [9] ログイン中の読者は、自分が付箋を貼った予測の一覧を専用の画面で見られる
- [10] 一覧の各項目には、予測の見出し・ジャンル・時間軸・付箋メモを表示する
- [11] 一覧の各項目から、元の記事詳細ページの該当予測へ移って読み返せる
...
### 文字数・件数
- [3] メモは200文字まで、1件の予測につき1読者1件までとする(news-digestと同じ)

## 依存関係
- 付箋の対象となる予測の識別子は[article-detail/requirements.md](../article-detail/requirements.md)の記事データに従う
