...
- Secretsは工程ごとに使う範囲を分離し、Claude CLIのヘッドレス実行ステップにはDB接続情報・GitHub PATのいずれも渡さない
  ...
  - コミット・push・PR作成ステップ(Claude CLI実行より後): [weekly-publish](../weekly-publish/design.md)と同じ`FUTURE_DIGEST_GH_PAT`を使う(このPRは自動マージしないため、自動マージの範囲と混ざらない)。このPATもClaude CLIのステップには渡さない
...
- 対象: 実行日から過去1か月分のデータ
- 手順:
  1. 過去1か月分の記事データを読み、候補が見つからなかった枠(ジャンル×時間軸)ごとに、その時間軸を扱った回のうち収集に失敗した回(`reason: 'collection-failed'`)を除いた回数(「有効回数」)と、そのうち候補なしだった回数を集計する(収集失敗は処理そのものが失敗しただけで採用基準の不足ではないため、候補なしの集計からも母数からも除く。content-selection/requirements.md#収集失敗-5)。生成に失敗した枠も別に数える(候補の不足ではないため)(requirements.md#見直しの実行-1)
  2. 有効回数が1回以上あり、そのすべての回で候補なしだった枠を「候補なしが続いている枠」として印を付ける。有効回数が0回(その時間軸を扱った回のすべてで収集に失敗した)枠には印を付けない(候補が本当に無かったのか判断できないため)【推測】(requirements.md#見直しの実行-5)
...
- 対象: 集めた材料
- 手順:
  1. 材料(直近1か月の運営者フィードバックが1件以上、または候補なしが続いている枠が1つ以上。収集失敗の枠は含めない)が1件もない月は、PRを作らずに終える(requirements.md#見直しの実行-4)
  ...
  4. 選定領域: フィードバックと収集状況をもとに、`content-selection/requirements.md`(ジャンル・採用基準・影響度の観点)と`content/future-digest/genres.json`(ジャンルの説明・注目テーマ)の変更案を作る。候補なしが続いている枠がある場合は、その採用基準・ジャンルの説明を確かめる変更案を必ず含める(requirements.md#見直しの実行-2・5)
  5. 生成領域: `content-generation/requirements.md`・`design.md`の変更案を作る。記事生成CLI(`scripts/future-digest/generate-content.ts`)はこの2ファイルを実行時に読み込んでプロンプトへ渡すため、CLI自体の変更は不要(次回の週次生成に自動で反映される)
  6. 著作権への配慮・性・恋愛ジャンルの書き方の制約を弱める変更は提案しない。そうした要望は変更案に入れず、却下したことと理由を表に残す(requirements.md#ビジネスルール・制約-3)
...
- 対象: 上記の変更
- 手順:
  1. Claude CLIがファイルの変更・テスト実行まで終えたら、ワークフロー(GitHub PATを使うステップ)がブランチ`future-digest/source-review/<年-月>`(例: `future-digest/source-review/2026-11`)を作ってコミット・pushし、`main`向けのPRを作る。本文は`/tmp/source-review-pr-body.md`を使う
