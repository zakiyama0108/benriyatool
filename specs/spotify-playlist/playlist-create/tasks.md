...
- 参照: design.md#曲名を一括検索する処理、design.md#エラーハンドリング
...
- 参照: design.md#曲名を一括検索する処理、design.md#状態管理、design.md#パフォーマンス
...
- 押下時に「対象曲(採用確定のみ)を入力順にまとめる→(ログイン時取得済みのユーザーIDで)プレイリスト作成→曲追加」の順に呼び出され、いずれかが失敗したら完了状態にならずエラー表示になることをテストする
- 参照: design.md#プレイリストを作成する処理
