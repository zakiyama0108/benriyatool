...
### 初期化中(判定中)
- 画面を開いた直後、ログイン状態の判定が完了するまでの間、全画面のローディング表示(スピナー等)のみを表示する。ゲート画面・曲名入力画面のどちらも表示しない(ログイン済みの再訪問者にゲート画面が一瞬ちらつくのを防ぐため)。ここでの「判定」は、URLに認可コードが付いている場合の「Spotifyでログインする処理」手順4以降(トークン交換・自ユーザー情報取得)、それ以外の場合の「ログイン状態を復元する処理」のいずれも含む
...
flowchart TD
    ...
    init -->|保存済みトークンなし、または復元・トークン交換に失敗| gate
