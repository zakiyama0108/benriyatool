...
- 手順:
  ...
  5. 戻ってきたstateが手順2で保存したものと一致することを確認する(一致しない場合は不正なリダイレクトとみなし、ログインを中断して未ログイン表示に戻す。この場合も手順6と同様にURLから認可コード・stateのクエリパラメータを除去し、手順2で一時保存したcode_verifier・stateを削除する)
  ...
  7. 取得したアクセストークンで自ユーザー情報(`GET /v1/me`)を取得し、ユーザーID・表示名・プロフィール画像を得る。このリクエストが通信エラー等で失敗した場合は、取得済みのトークンを保存せずログイン処理全体を失敗として扱い、未ログイン表示に戻した上で、手順6と同様にURL・sessionStorageの後始末を行う
  ...
  10. 利用者がSpotify側で認可を拒否した場合(認可コードの代わりに`error`パラメータを付けて画面に帰還した場合)、ログインを中断し未ログイン表示に戻す(通常のキャンセル操作として扱い、エラー表示はしない)。この場合も手順6と同様にURLから`error`・stateのクエリパラメータを除去し、手順2で一時保存したcode_verifier・stateを削除する
- シーケンス図:
```mermaid
sequenceDiagram
    actor user as 利用者
...
- プレイリスト作成APIがプレイリスト名の文字数超過(または不正な文字列)を理由に失敗した場合(400系のバリデーションエラー)、再試行を促す上記の汎用メッセージではなく「プレイリスト名を短くするか変更してお試しください。」等、入力の見直しを促す専用メッセージを表示する(requirements.md#プレイリスト名-2)
