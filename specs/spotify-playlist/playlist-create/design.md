...
- 手順:
  ...
  5. 戻ってきたstateが手順2で保存したものと一致することを確認する(一致しない場合は不正なリダイレクトとみなし、ログインを中断して未ログイン表示に戻す。この場合も手順8と同様にURLから認可コード・stateのクエリパラメータを除去し、手順2で一時保存したcode_verifier・stateを削除する)
  6. 認可コードとcode_verifierを使ってSpotifyのトークン発行エンドポイントにアクセスし、アクセストークン・リフレッシュトークン・有効期限を取得する(クライアントシークレットは使わない)。このリクエストが通信エラー等で失敗した場合は、未ログイン表示に戻した上で、手順8と同様にURLから認可コード・stateのクエリパラメータを除去し、手順2で一時保存したcode_verifier・stateも削除する(失敗時も認可コードをURL・ブラウザ履歴に残さないため)
  7. 取得したアクセストークンで自ユーザー情報(`GET /v1/me`)を取得し、ユーザーID・表示名・プロフィール画像を得る。このリクエストが通信エラー等で失敗した場合は、取得済みのトークンを保存せずログイン処理全体を失敗として扱い、未ログイン表示に戻した上で、手順8と同様にURL・sessionStorageの後始末を行う
  ...
  10. 利用者がSpotify側で認可を拒否した場合(認可コードの代わりに`error`パラメータを付けて画面に帰還した場合)、ログインを中断し未ログイン表示に戻す(通常のキャンセル操作として扱い、エラー表示はしない)。この場合も手順8と同様にURLから`error`・stateのクエリパラメータを除去し、手順2で一時保存したcode_verifier・stateを削除する
- シーケンス図:
```mermaid
sequenceDiagram
    actor user as 利用者
...
- 手順:
  ...
  5. 検索結果が複数件の曲は、候補未確定のまま上位5件を一覧表示する(requirements.md#候補の確定方法-2)。6件目以降がある場合は「もっと見る」を表示し、押されるたびに次の5件を追加取得してその曲の一覧に追加する。取得済みの件数がSpotify側の全候補数に達したら「もっと見る」を非表示にする
...
- 手順:
  ...
  3. まだプレイリストを作成していない場合のみ、利用者自身のSpotifyアカウントに非公開のプレイリストを新規作成し、作成したプレイリストIDを保持する(requirements.md#プレイリストの公開範囲-1)。既に作成済み(手順6により曲の追加だけが失敗して再実行された場合)はこの手順をスキップし、保持しているプレイリストIDをそのまま使う(Spotifyのプレイリストはアプリから削除できないため、再実行のたびに新規作成すると空のプレイリストが利用者のアカウントに残留・蓄積してしまう。それを防ぐための設計)。この保持状態は画面表示用にメモリ内で保持するのみでブラウザには保存しないため、プレイリスト作成後・曲追加前に利用者がページを再読み込み・離脱した場合も同様に空のプレイリストが残りうるが、明示的な失敗時と同じ残存リスクとして許容する
  4. 保持しているプレイリストIDに対し、対象の曲を入力順に追加する(対象曲は最大100件(requirements.md#曲名の入力-2)のため、Spotifyの曲追加APIの1リクエストあたりの上限内に収まり、分割呼び出しは不要)
  5. 作成が完了したら、作成したプレイリストへのリンクを表示する完了状態に切り替える(requirements.md#機能要件-9)
  6. 途中の手順(プレイリスト作成・曲の追加)のいずれかが失敗した場合、完了状態にはせず、エラーを表示して利用者が再度「プレイリストを作成」をやり直せるようにする(自動リトライはしない)。手順3でプレイリストが作成済みの状態で失敗した場合は、そのプレイリストIDを保持したままにし、再実行時は手順4(曲の追加)からやり直す
...
- プレイリスト作成APIがプレイリスト名を理由とするバリデーションエラー(400系)で失敗した場合、再試行を促す上記の汎用メッセージではなく「プレイリスト名を短くするか変更してお試しください。」等、入力の見直しを促す専用メッセージを表示する(requirements.md#プレイリスト名-2)
...
### 初期化中(判定中)
- 画面を開いた直後、ログイン状態の判定が完了するまでの間、全画面のローディング表示(スピナー等)のみを表示する。ゲート画面・曲名入力画面のどちらも表示しない(ログイン済みの再訪問者にゲート画面が一瞬ちらつくのを防ぐため)。ここでの「判定」は、URLに認可コードまたは`error`パラメータが付いている場合の「Spotifyでログインする処理」手順4以降(トークン交換・自ユーザー情報取得、または拒否時の後始末)、それ以外の場合の「ログイン状態を復元する処理」のいずれも含む
- 判定完了後、結果に応じて「未ログイン(ゲート)」または「曲名入力・検索結果表示」に切り替える
...
- 利用者が入力する曲名・プレイリスト名をSpotify検索API・プレイリスト作成APIへ渡す際は、クエリパラメータ・JSONのいずれも標準のエンコード関数(`encodeURIComponent`・`JSON.stringify`)を用いる(自前の文字列結合でエスケープを省略しない)
...
- 上記いずれのログにも、ユーザーID・表示名・プロフィール画像・曲名・プレイリスト名等のPIIは含めない(件数・ステータスコード・手順名など非個人情報のみを出力する)
