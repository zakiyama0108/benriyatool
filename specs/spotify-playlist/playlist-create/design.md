...
- 手順:
  ...
  8. 取得したトークン一式をブラウザに保存し、URLから認可コード・stateのクエリパラメータを取り除き(ブラウザ履歴に認可コードを残さないため)、手順2でsessionStorageに一時保存したcode_verifier・stateも削除する。ユーザーID・表示名・プロフィール画像は画面表示用にメモリ内(`page.tsx`の状態)にのみ保持し、ブラウザには保存しない(design.md#セキュリティ)
...
- 対象: 曲名入力欄の内容、「検索する」操作
- 手順:
  1. 入力欄の内容を改行で分割し、空行を除いた曲名の一覧を作る(重複除去はしない、requirements.md#曲名の入力-3)。100件を超える場合は上位100件のみを対象とし、101件目以降は無視する(requirements.md#曲名の入力-2)。無視した曲名がある場合は、その旨(101件目以降が検索対象外であること)を画面に表示する
