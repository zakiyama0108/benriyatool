...
- 🔴 各分類の絞り込み(対応人数の範囲内包/プレイ時間の重なり/ジャンルの配列包含(`genres`に含まれるか)/対象年齢・難易度・メーカー一致/言語依存度/受賞歴あり/作者の部分一致)がAND条件で効くこと、複数ジャンルを持つゲームが指定ジャンルを含んでいれば残ることを個別にテストすること、未登録項目はその分類指定時に除外されることをテストする。選択肢組み立ては、ジャンル・難易度・メーカー・対象年齢が取得データの値に基づくこと、対応人数・プレイ時間は固定の代表値であることをそれぞれテストする
- 🟢 取得済みゲームへ絞り込み条件を適用する純粋関数と、選択肢組み立て(ジャンル・難易度・メーカー・対象年齢は動的、対応人数・プレイ時間・言語依存度・受賞歴は固定)を実装する
- 🔵 条件適用の分岐を整理する
