...
- 手順(指定された条件をすべて満たすゲームに絞る。AND条件。requirements.md#絞り込み-6):
  ...
  3. **ジャンル**: ドロップダウンから1つのジャンルを指定した場合、そのジャンルが各ゲームの`genres`(複数選択可、[game-registration/design.md#データベース設計](../game-registration/design.md))に含まれるゲームだけを残す(1ゲームが複数ジャンルを持ちうるため、値の一致ではなく配列への包含で判定する。他の分類のような単一値の一致判定とは異なる)
