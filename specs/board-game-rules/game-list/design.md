...
- 手順:
  1. ジャンル・難易度・メーカー/出版社・対象年齢は、取得済みの公開中ゲームが実際に持つ値から選択肢を組み立てる(登録されている値に基づく。requirements.md#表示対象-2)。値を持つゲームがない分類は選択肢が空になる
  2. 言語依存度は「あり/なし」、受賞歴は「該当のみ」の固定の選択肢とする
  3. 対応人数・プレイ時間は範囲型(下限〜上限)のカラムのため、他分類のような実データの値からの選択肢組み立ては行わず、あらかじめ用意した固定の代表値(対応人数の候補・時間帯の候補)を選択肢とする(具体的な刻み値はUIの実装時に決める)
