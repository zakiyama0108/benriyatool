...
- 手順(指定された条件をすべて満たすゲームに絞る。AND条件。requirements.md#絞り込み-6):
  1. **対応人数**: 「この人数で遊べる」を指定した場合、その人数がゲームの対応人数の下限〜上限の範囲に収まるゲームだけを残す。`min_players`・`max_players`のいずれかが未登録(NULL)のゲームは、この分類で絞り込みを指定したときは結果から除外する(原則。手順10。ローカル生成が根拠のない値を推定しない方針のため未登録がありうる。[admin/requirements.md#登録実行のローカル処理起動-13〜14](../admin/requirements.md))
  2. **プレイ時間**: 希望する時間帯を指定した場合、その時間帯とゲームのプレイ時間の下限〜上限が重なるゲームだけを残す(時間帯の境界(下限・上限)が接するだけの場合も重なりに含める。対象年齢の境界値の扱い(手順4)と揃える)。`min_minutes`・`max_minutes`のいずれかが未登録(NULL)のゲームも同様にこの分類の絞り込み時は除外する
  3. **ジャンル**: ドロップダウンから1つのジャンルを指定した場合、そのジャンルが各ゲームの`genres`(複数選択可、[game-registration/design.md#データベース設計](../game-registration/design.md))に含まれるゲームだけを残す(1ゲームが複数ジャンルを持ちうるため、値の一致ではなく配列への包含で判定する。難易度・メーカー・言語依存度のような単一値の一致判定とは異なる。requirements.md#絞り込み-6)
...
- 手順(分類によって「固定の代表値」と「取得データから動的に組み立てる」の2方式を使い分ける。requirements.md#表示対象-2):
  1. **難易度・メーカー/出版社**: 取得済みの公開中ゲームが実際に持つ値から選択肢を動的に組み立てる(登録されている値に基づく)。値を持つゲームがない分類は選択肢が空になる。理由: 登録フォーム([game-registration/design.md](../game-registration/design.md))・admin編集フォームがいずれも自由テキスト入力で固定語彙を持たないため、固定リスト化すると実際の登録値と一致せず機能しない
  2. **ジャンル**: `app/board-game-rules/lib/genres.ts`の`GENRES`(DB CHECK制約・登録フォームと同じ固定28種)を、取得データに関わらず常に全件選択肢として提示する(動的な組み立ては行わない)。理由: 固定リストが既にあり、取得データの実際の値に選択肢が左右されない方が探しやすい(画面レビューでの方針決定、2026-08)
  3. **対象年齢**: 対応人数・プレイ時間と同じく、範囲型カラム(`min_age`は単一値だが「◯歳以上」の閾値として扱う)に対する「あらかじめ用意した固定の代表値」を選択肢とする(取得データからの組み立ては行わない)。候補は`MIN_AGE_OPTIONS`(`app/board-game-rules/lib/filterGames.ts`): 3/6/8/10/12歳以上/15歳以上の6段階
  4. 言語依存度は「あり/なし」、受賞歴は「該当のみ」の固定の選択肢とする
  5. 対応人数・プレイ時間は範囲型(下限〜上限)のカラムのため、実データの値からの選択肢組み立ては行わず、あらかじめ用意した固定の代表値(対応人数の候補・時間帯の候補)を選択肢とする。プレイ時間の帯は`PLAYTIME_BAND_OPTIONS`(同ファイル): 15分以内/30分以内/30〜60分/60〜90分/90〜120分/120分以上の6段階(「15分以内」は「30分以内」と範囲が重なるが、絞り込みロジックの重なり判定とは独立した選択肢候補のため問題ない。画面レビューでの追加依頼、2026-08)
...
- 件数表示: 現在の条件に一致する件数
- 一覧(`GameCard`のカード形式): カード上部にメイン画像(またはプレースホルダー)を全幅で表示し、その下にゲーム名・対応人数・プレイ時間・ジャンルを最低限表示する。対応人数・プレイ時間が未登録(NULL)の場合は「不明」と表示する(項目自体は常に1行として出す。お気に入り一覧(`favorites/page.tsx`)も同じ表示)。ログイン中はお気に入りの登録・解除操作を画像の右上に重ねて表示する(requirements.md#一覧表示-4、[favorite](../favorite/requirements.md))。カードから詳細画面へ遷移できる(requirements.md#一覧表示-2)
