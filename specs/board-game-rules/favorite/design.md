...
- ログイン中1件以上: 登録日時の新しい順の3〜4列カードグリッド。各カードにゲーム名・対応人数・プレイ時間・ジャンルチップ・その場での解除操作(カード右上のお気に入りボタン)・詳細へのリンク(カードクリック)。対応人数・プレイ時間が未登録(NULL)の場合は「不明」と表示する([game-list/design.md](../game-list/design.md)の`GameCard`と同じ表示)
