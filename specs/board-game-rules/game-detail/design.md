...
- 手順:
  ...
  2. そのIDの公開中(`deleted_at is null`)ゲームを取得する。`photo_paths`(元写真パス)は取得しない(元写真は詳細に一切表示しない。requirements.md#表示対象-2、[game-registration/design.md#セキュリティ](../game-registration/design.md))。`intro_photo_paths`(ゲーム紹介画像、公開列)は取得する
...
## 運営者向けの操作(管理者ログイン時)
方向B(2026-08-19合意)により、旧・管理画面のゲーム一覧を廃止し、ゲーム個別のモデレーション操作をこの詳細画面に集約する([admin/design.md](../admin/design.md))。運営者(管理者)ログイン時のみ、閲覧表示に加えて以下の管理者導線を表示・実行できる。
...
- 補足(`deleted_at`の扱い): 物理削除への統一に伴い論理削除は使わないが、既存の`deleted_at`列と公開取得RLS`deleted_at is null`は当面そのまま残す(物理削除では常に`deleted_at is null`が真になり無害)。列のDROPはfavorites・game-list・game-detailの取得条件やコードに波及するため本specでは行わない(将来の別スコープ)。【推測】この残置方針は最小リスクを優先した判断で、仕様承認レビューで確認する
- 関連するビジネスルール: requirements.md#運営者向けの操作-11、requirements.md#運営者による削除の方針

#### 物理削除のDB設計(新規マイグレーション)
既存スキーマは論理削除前提のため、物理削除には次の変更を新規マイグレーションで加える:
1. `board_game_rules_games`に運営者本人のみの**DELETEポリシー**を追加する(現状は`admin can update games`のみでDELETEポリシーがない):
```sql
...
```
2. 子テーブルの`game_id`外部キーを`ON DELETE CASCADE`に付け替える(現状は`references board_game_rules_games(id)`のみでカスケードなし。このままではゲーム行DELETEがFK違反で失敗する)。対象: `board_game_rules_comments`・`board_game_rules_favorites`・`board_game_rules_reports`。各テーブルで既存FK制約をDROPし、`on delete cascade`付きで再作成する。FKのカスケードは参照アクションのため子テーブルのRLSに関係なく連動削除される(運営者が通報・お気に入りのDELETE RLSを持たなくても、ゲーム削除に伴い子行が消える)
...
- 手順(旧admin/design.mdの同処理を移設。挙動は同一):
...
- 管理者の編集・物理削除・紹介画像差し替え・コメント削除・元写真照合は運営者が明示的に行う操作のため、失敗時は失敗が分かる表示をする。編集は入力を保持する。処理中は該当操作を無効化し二重実行を防ぐ(旧admin/design.mdの方針を踏襲)
...
# 方向Bで管理画面から移設(運営者ログイン時のみ)
app/board-game-rules/components/AdminControls.tsx (新規: 管理者導線パネル。編集フォーム展開・物理削除(確認付き)・元写真照合をまとめる。旧admin/GameEditForm.tsxの編集フォームを流用・再構成)
app/board-game-rules/lib/gameModeration.ts (新規: ゲーム編集UPDATE・物理DELETE・コメント削除。旧admin/lib/moderation.tsを移設し、論理削除→物理削除に変更)
app/board-game-rules/lib/originalPhotos.ts (新規: 非公開Storageから元写真を取得。旧admin/lib/photos.tsを移設)
app/board-game-rules/lib/introPhotos.ts (移設: ゲーム紹介画像の追加/削除/並び替え。旧admin/lib/introPhotos.ts)
...
- **管理者導線(運営者ログイン時のみ、`AdminControls`、新規)**: 分類情報付近に、編集・物理削除・元写真照合の操作をまとめたパネルを出す。編集は分類情報・ルール本文の編集フォーム(旧`GameEditForm`相当)を展開して上書き保存、削除は確認ステップ付きの物理削除、元写真照合は非公開Storageの元写真を運営者のみ取得して表示する。紹介画像ギャラリー(`PhotoGallery`)は運営者ログイン時のみ差し替え・削除・並び替えのUIを併せて出す。コメント欄(`CommentSection`)は運営者ログイン時のみ各コメントに削除導線を出す。これらは一般の閲覧者・ログイン利用者には一切出さない(requirements.md#運営者操作のアクセス制御-7)。【推測】具体的な配置・見た目(パネルの位置、編集フォームの展開方法)は実装時に画面幅で調整する軽微なUIで、既存デザイントークンの範囲に収める
...
## セキュリティ
- 取得は公開中(`deleted_at is null`)のゲームに限られ、削除済み(物理削除でレコード自体が消える)は表示しない(RLSでも担保。requirements.md#表示対象-1)
...
- 管理者操作(編集・物理削除・紹介画像差し替え・元写真取得・コメント削除)が想定外に失敗した場合も、原因究明のためコンソールにエラーを出す。ログにはゲーム情報・写真・コメント本文の中身を含めず、失敗の事実・種別にとどめる(旧admin/design.mdの方針を踏襲)
...
- 運営者向けの操作は方向Bで管理画面([admin/design.md](../admin/design.md))から移設したもの。管理者判定・認証(共用`adminAuth.ts`・`admin_emails`)、games/comments/Storageの運営者向けRLS・ポリシーは[admin/design.md](../admin/design.md)を正とする。物理削除に必要なgames DELETEポリシー・子FKの`ON DELETE CASCADE`マイグレーションは本design.md「物理削除のDB設計」で新規に定義する
