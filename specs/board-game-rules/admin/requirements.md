...
- [8] 依頼の写真・入力済み分類情報をもとに、管理画面から登録処理を起動できる(詳細は下記「登録実行・下書きレビュー」)。写真解析・ルール本文の生成そのものは運営者のローカル環境で行われる
- [9] 不要な依頼(スパム・重複・情報不足など)を削除できる

### 登録実行・下書きレビュー
- [1] 未処理の登録依頼に対して「登録実行」を押すと、ローカル環境での解析処理(写真解析・分類情報とルール本文の生成)が起動する。押した時点で処理が完了するわけではないため、進行中であることが分かる表示をする
- [2] 依頼ごとの処理状況(未着手/処理中/下書きあり/公開済み/失敗)を一覧で確認できる
- [3] 処理が完了すると下書き(生成された分類情報・ルール本文)が確認できるようになる。ゲーム名・対応人数・プレイ時間・ジャンル・簡単版ルールの抜粋などを確認できる
- [4] 下書きに対して、次のいずれかを選べる:
  ...
  - **破棄**: 上記「登録依頼の確認」[9]の依頼削除の操作を使う(下書き専用の別操作は設けない)
- [5] 過去に依頼した再調整の要望を、下書きの履歴として新しい順に確認できる
- [6] 処理が失敗した場合、失敗した旨と原因(分かる範囲)を表示する。「登録実行」を再度押すことで再試行できる
...
- [11] 「公開する」操作は、運営者本人のログインセッションから直接ゲームとして登録される(ローカル処理を経由せず、押した時点で即時公開される)。これは既存の「編集・削除」と同じ、運営者本人に限定した書き込み例外([docs/adr/0007-runtime-llm-server-and-writable-admin.md](../../../docs/adr/0007-runtime-llm-server-and-writable-admin.md))の延長である
