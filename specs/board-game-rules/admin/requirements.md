...
> ステータス: 仕様確認中(未実装) — 本ステータスは追記した[18](共通ナビへの管理画面導線)の実装着手までの一時的なもの。[1]〜[17]は実装済み
...
- [4] ログアウトできること
- [18] 運営者本人としてログインしている場合に限り、board-game-rulesの共通ナビ(左サイドバー)に管理画面(`/board-game-rules/admin/`)への導線を表示する。未ログイン・運営者以外のアカウントでは表示しない。これは運営者が管理画面へ素早く到達するための利便であって、アクセス制御ではない(保護は下記[ビジネスルール2]のRLSが担い、導線の表示有無にかかわらず未権限者は管理機能を利用できない)。運営者判定は既存の共通認証(`app/lib/adminAuth.ts`の`isAuthorizedAdmin`、`admin_emails`許可リスト)を再利用し、新規のロジックは持たない
