...
- 手順:
  ...
  3. **下書き確認**: `status`が`draft`になったら、`draft_content`を運営者が公開可否を判断できる粒度で表示する(requirements.md#登録実行・下書きレビュー-18)。ゲーム名・対応人数・プレイ時間・ジャンルに加え、簡単版ルールの全文、詳しい版ルール(`rulesChapters.ts`の共通章立て順で全章。本文が空の章も見出しを出し「(記載なし)」と示す)、分類情報(対象年齢・難易度・出版社・作者・日本語ルールの有無・受賞歴・発売年のうち値があるもの)を表示する。詳しい版ルールはカードが縦に伸びすぎないよう折りたたみ表示にしてよい
...
- 補足: 入力となる写真は匿名アップロードのため攻撃者が内容を完全に制御できる前提で扱う。ヘッドレス`claude -p`は最小権限で起動し(危険なツールを付与しない・作業ディレクトリを隔離しリポジトリへの書き込みを許さない)、`SUPABASE_SERVICE_ROLE_KEY`等の資格情報を持つプロセスから画像内の埋め込みテキストによって意図しない操作が行われないようにする。公開前の運営者レビュー(手順4「公開する」)は生成内容の品質を担保するものであって、プロンプトインジェクション対策そのものではない(下記「セキュリティ」)
...
- 登録依頼一覧(`GameRequestsView`): 未処理を優先・次いで新しい順。写真プレビュー・ゲーム紹介画像プレビュー(0枚なら「紹介画像なし(登録時に自動補完されます)」の案内)・入力済み分類情報を表示。写真プレビュー(元写真の署名付きURL・ゲーム紹介画像の公開URL)はサムネイルをクリックすると原寸画像を別タブで開ける(AI生成物と元写真の突き合わせのため。モーダルは設けない)。処理済み/未処理バッジ(`processed_at`をそのまま表示)と削除の導線。各依頼は`status`(未着手/処理中/下書きあり/公開済み/失敗)のバッジと、状態に応じた操作(`DraftReviewCard`)を持つ:
  ...
  - 下書きあり: 下書きの内容(ゲーム名・対応人数・プレイ時間・ジャンル・分類情報・簡単版ルール全文・詳しい版ルール全章(折りたたみ可))、「公開する」「再調整を依頼」ボタン、再調整の要望入力欄、再調整履歴(新しい順)
