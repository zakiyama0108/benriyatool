...
## 修正: 一覧内でアクティブなシナリオを分かるようにする(2026-08)

## T15. ScenarioPanelの一覧にアクティブなシナリオの視覚的な目印を追加(仕様: requirements.md#上書き保存-16、design.md#画面設計)
- 対象ファイル: `app/life-money-sim/components/ScenarioPanel.tsx`、`__tests__/life-money-sim/components/ScenarioPanel.test.tsx`
- 内容:
  - 🔴 一覧内でアクティブなシナリオの行に「編集中」バッジが表示され、それ以外の行には表示されないことを確認するテストを書く
  - 🟢 実装する(行の背景色・枠線も変える)
- `/run`(run-benriyatoolスキル)で、名前欄の内容と一覧内の「編集中」表示が同じシナリオを指していることが見た目で分かることを実機確認する
