...
- 手順:
  1. ログイン中でない場合、マイシナリオの操作(名前を付けて保存・保存済み一覧)自体を表示せず、代わりに「この試算を送信する」ボタンの近くにログイン誘導の案内(文言例: 「ログインすると賞与・イベント名目や生年月も含めて全項目を保存・復元できます」+ログインボタン)を表示する【推測: 文言の具体的な表現】
  2. ログイン中の場合、既存の「この試算を送信する」(匿名保存)とは別の部品として、マイシナリオの操作を表示する(ログイン誘導の案内は表示しない)
...
- 手順:
  ...
  4. 上書き更新と判定した場合、実行前に対象のシナリオ名を含む確認ダイアログ(文言例:「『{シナリオ名}』を更新します。よろしいですか?」)を表示する【推測: ダイアログの具体的な文言・実装(`window.confirm`で簡易的に実装してよいと判断)】。利用者が確認した場合のみ、対象行を現在の入力値一式でUPDATEする(名前は変更しない)
...
- 対象: ログイン中に「この試算を送信する」(匿名保存)を実行する場合
...
app/life-money-sim/components/ScenarioLoginPrompt.tsx (既存: 未ログイン時に「この試算を送信する」ボタンの近くへ表示する、ログイン誘導の案内。文言中の参照ボタン名を変更)
...
## 画面設計
- 「マイシナリオ」パネルを、既存の「この試算を送信する」ボタンとは別の部品として配置する(ヒアリングで確定した「別パーツ」案)
- ログイン中のみ表示。未ログインでは表示自体をしない
- パネルの内容:
  - シナリオ名の入力欄と保存ボタン。アクティブなシナリオがあり名前欄がそのシナリオの名前のままの場合はボタン文言を「更新する」、それ以外は「保存する」と表示する【推測: 文言・切り替え条件の具体的な表現】
