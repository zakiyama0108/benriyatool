...
- 手順:
  1. ログイン中でない場合、マイシナリオの操作(名前を付けて保存・保存済み一覧)自体を表示しない
  2. ログイン中の場合、既存の「この試算を保存する」(匿名保存)とは別の部品として、マイシナリオの操作を表示する
...
- 収入(`IncomeInput`)、個人支出の内訳(`PersonalExpenseInput`)、家計支出の内訳(`HouseholdExpenseInput`)、家族構成(`FamilyProfileInput`)、開始資産額・開始年月(`StartingAssetInput`)、賞与の登録内容(`BonusEntry[]`)、イベントの登録内容(`EventEntry[]`)、貯蓄のみ/資産運用モードと想定利回り(`InvestmentModeInput`)
...
app/life-money-sim/page.tsx (既存: 保存対象の入力値一式を保持する状態に、読み込み時の一括置き換え処理を追加)
