...
- [ ] Task 6: 内訳リスト入力コンポーネント(仕様: requirements.md#個人支出-1、#個人支出-2、design.md#コンポーネント設計)
...
- [ ] Task 7: 収入入力・家計負担入力コンポーネント(仕様: requirements.md#収入、#家計支出、design.md#コンポーネント設計)
  - [ ] `app/life-money-sim/components/IncomeForm.tsx`、`HouseholdShareInput.tsx`を実装する(配偶者なしの場合は家計支出セクションを表示しない)

- [ ] Task 8: 収支タブの画面配線(仕様: design.md#画面設計、#状態管理)
  - [ ] `app/life-money-sim/page.tsx`に「収支」タブを実装し、Task1〜7の関数・コンポーネントを配線して`BalanceSummary.tsx`に結果を表示する
