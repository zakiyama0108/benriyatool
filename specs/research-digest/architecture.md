...
| [bookmark](bookmark/requirements.md) | 気になった研究を付箋で残す | ログインした読者が記事ごとにメモ付きの付箋を貼り、一覧で見返す | article-detailの記事識別子に従う | 仕様のみ(未実装) |
...
| サービス | 用途 |
|---|---|
| Supabase(`research_digest_feedback`・`research_digest_bookmarks`テーブル) | 運営者フィードバック・読者の付箋の保存 |
| Supabase Auth(Google OIDC) | 読者のログイン(付箋)、運営者判定(フィードバック欄の表示切り替え・INSERT権限) |
| GitHub Actions | 週次の記事生成・月次見直し・LINE配信の実行基盤 |
...
使用テーブルは2つ(`research_digest_feedback`・`research_digest_bookmarks`)で、両テーブルとも記事JSON(DB外)の`article_id`・`finding_id`を参照するのみでテーブル間の外部キー関係はない。
```mermaid
erDiagram
