...
## サマリ
月に1回、運営者のフィードバックと収集状況(掲載を見送ったジャンル・候補件数)をもとに、ジャンル・採用基準([content-selection](../content-selection/requirements.md))と執筆ルール([content-generation](../content-generation/requirements.md))の見直し案をPRで出す。反映は運営者が確認して承認(マージ)してから行う。trend-digestのsource-reviewと同じ運用にする。
...
- 運営者として、候補が見つからず掲載を見送るジャンルが続いていることに気づき、基準を見直したい
...
- [5] 候補が見つからず掲載を見送ったジャンルが直近1か月続いている場合は、その採用基準を確かめる見直し案を必ず含める
