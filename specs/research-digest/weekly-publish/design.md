...
- 手順:
  ...
  3. 採用した候補が1件もない場合、空になったジャンルがすべて候補なしであれば、記事を作らず後述「公開をスキップする処理」に進む(requirements.md#掲載件数の保証-3)。収集失敗のジャンルが1つ以上混在する場合は、記事を作らず後述「収集失敗で実行を失敗させる処理」に進む(requirements.md#掲載件数の保証-3)【推測】
