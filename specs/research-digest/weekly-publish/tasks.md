...
- Task 1: 記事データの組み立て(仕様: requirements.md#掲載件数の保証-1・2・4、design.md「1回分の記事を生成する処理」手順5)
...
- Task 3: ワークフロー本体(仕様: design.md「実行環境の前提」「処理フロー」)(TDD対象外。GitHub Actionsの定義のため。future-digest-weekly.ymlと同じ構造で実装する)
  ...
  - `publish`ジョブ: `RESEARCH_DIGEST_GH_PAT`でcheckout→Claude Code CLIのインストール→実行日(JST)の算出→ブランチ作成→`collect-and-select.ts`→採用0件で空いたジャンルがすべて候補なしならスキップ(成功で終了)/収集失敗が1つ以上混在すれば失敗(非ゼロ終了)→`generate-content.ts`→`write-article.ts`→コミット・push・PR作成・`gh pr merge --auto --squash`
