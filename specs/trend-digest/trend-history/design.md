...
- 削除・自動アーカイブは行わない(requirements.md#履歴データの保持期間-14)
...
export type Observation = {
  ...
  originRegion: string | null // 発祥地域。判定できない場合はnull(=不明。requirements.md#地域情報-9)
...
export type CandidateHistory = {
  ...
  strengthSeries: Array<{ date: string; strength: number }> // 日付昇順の強度の推移
...
export type StatusJudgement = {
  ...
  isPublishable: boolean // EMERGING/GROWING/ESTABLISHED/STABLEのいずれかか(requirements.md#ステータス判定基準-8)
}
```
...
```ts
// app/trend-digest/lib/watchlistTypes.ts のCriteriaに追加する
export type HistoryCriteria = {
  shortTermMaxDays: number // SHORT_TERMとみなす継続日数の上限(requirements.md#ステータス判定基準-2 = 13)
  growingMinDays: number // GROWING判定の継続日数の下限(同-4 = 14)
  establishedMinDays: number // ESTABLISHED判定の継続日数の下限(同-5 = 30)
  stableMinDays: number // STABLE判定の継続日数の下限(同-6 = 90)
...
倍率(比)で判定し、強度の差の絶対値では判定しない。強度の尺度が選定方式によって桁違い(固定リストジャンルは`100 - 順位`でおおむね80〜99、WebSearchジャンルは独立情報源数で2〜5)であり、絶対値の閾値を1つに決めると一方の方式でしか機能しないため。
...
- 手順:
  ...
  2. 同じ正規化タイトルの候補が同じ回に複数のジャンル・情報源から取れた場合は、強度が最も大きい1件だけを残す(同じ回の同じ話題を二重に数えると、検知した実行回数と強度の推移が実態より大きくなるため)
...
- 手順:
  ...
  3. 発祥地域・現在の主な流行地域は、情報源の記述から判定できた場合のみ記録する。判定できない場合は発祥地域を「不明」(値なし)、主な流行地域を「不明」(空)として扱い、推測で埋めない(requirements.md#地域情報-9)
  4. 日本での強度・海外での強度も、判定できない場合は「不明」(値なし)として扱う。0件だったことと、判定できなかったことを区別する(慢性的に判定できていない状態を月次見直しで拾えるようにするため)
  5. 比率を目安の配分(日本8割・海外2割)へ寄せる調整は行わない(requirements.md#地域情報-11)
- 関連するビジネスルール: requirements.md#機能要件-5、requirements.md#地域情報-9〜11
...
- 手順:
  ...
  3. 系列ごとに、初回検知日(最も古い観測の実行日)・直近検知日(最も新しい観測の実行日)・検知した実行回数(観測が存在する実行の数)・強度の推移(実行日昇順の強度の並び)を求める
  4. 地域情報・ジャンル・原題は、直近の観測のものを採る(最新の状況を表すため)。ただし発祥地域は最も古い観測で判定できたものを優先して残す(「最初に流行が確認された地域」という定義上、後の回で不明になっても失いたくないため。requirements.md#地域情報-9)
  5. 直近の実行(今回の観測ログ)にその候補が含まれていない場合は「言及が途絶えた」状態として扱う(requirements.md#機能要件-3)
...
- 手順:
  ...
  2. 下記の順に条件を当てはめ、最初に当てはまったものをその候補のステータスとする。上から順に当てはめるのは、要件の各条件が重なる範囲を持つため(例: 継続日数が20日で増加傾向のある候補はEMERGINGの条件とGROWINGの条件を同時に満たす)
     1. 継続日数が0の場合はNEW(requirements.md#ステータス判定基準-1)
     2. 直近の実行で検知されなかった(言及が途絶えた)場合で、継続日数がSHORT_TERMの上限日数以下ならSHORT_TERM(同-2)
     3. 直近の実行で検知されなかった場合で、継続日数がSHORT_TERMの上限日数を超えているならDECLINING(同-7。検知されなくなったことは「直近の強度がピークから最も大きく減少した」状態にあたるため、DECLININGとして扱う。要件が明示していない範囲を、要件の趣旨に沿って設計で補った判断)
     4. 直近の強度がピーク強度から明確に減少している場合はDECLINING(同-7)。「明確に減少」は、直近の強度をこの系列のピーク強度で割った値が`decliningRatio`以下で、かつピークが直近の観測ではないこと(直近がピーク自身であれば減少していないため)。観測が`minSamplesForTrend`回未満の系列では判定しない(数回の観測ではぶれと減少を区別できないため)
     5. 継続日数がSTABLEの下限日数以上で、強度が大きく増減せず一定を保っている場合はSTABLE(同-6)。「一定を保っている」は、直近`minSamplesForTrend`回分の強度がいずれもその平均の上下`stableBandRatio`の幅に収まっていること
     6. 継続日数がESTABLISHEDの下限日数以上の場合はESTABLISHED(同-5)
     7. 継続日数がGROWINGの下限日数以上で、強度が明確に増加傾向にある場合はGROWING(同-4)。「明確に増加傾向」は、強度の推移を前半と後半に二分し、後半の平均を前半の平均で割った値が`risingRatio`以上であること。観測が`minSamplesForTrend`回未満の系列では判定しない
     8. 上記のいずれにも当てはまらず、直近の実行でも検知されている場合はEMERGING(同-3)
  3. ステータスがEMERGING・GROWING・ESTABLISHED・STABLEのいずれかの場合のみ「掲載可能」と判定する(同-8)
- 状態遷移図(俯瞰用。正は上記の手順の文章。判定は毎回すべての観測ログから再計算するため、この図は「前の状態から遷移する」のではなく「継続日数と強度の推移が変わった結果どのステータスに移りうるか」を表す):

```mermaid
stateDiagram-v2
    [*] --> NEW: 今回初めて検知
    NEW --> EMERGING: 次回も検知（継続中）
    NEW --> SHORT_TERM: 13日以内に途絶えた
    EMERGING --> GROWING: 14日以上 かつ 強度が増加傾向
    EMERGING --> SHORT_TERM: 13日以内に途絶えた
    ...
    EMERGING --> DECLINING: 強度がピークから明確に減少
    GROWING --> DECLINING: 強度がピークから明確に減少
    ESTABLISHED --> DECLINING: 強度がピークから明確に減少
    STABLE --> DECLINING: 強度がピークから明確に減少
    DECLINING --> EMERGING: 強度が持ち直した
    SHORT_TERM --> [*]
```

- 関連するビジネスルール: requirements.md#機能要件-4、requirements.md#ステータス判定基準-1〜8

### 掲載実績(報告回数・前回掲載時のステータス)を求める処理
- 対象: `content/trend-digest/articles/`配下の全記事の全トピック
- 手順:
  1. 全記事のトピックを、対象作品・話題の原題を正規化したものごとにまとめ、発行日の昇順に並べる
  2. 候補ごとに、過去に掲載された回数を数える。今回掲載する場合の「何回目の報告か」は、この回数に1を足した値とする(requirements.md#掲載実績の追跡-12)
  3. 直近で掲載されたときのステータスを、最も新しい掲載トピックが持つステータスから取る。ステータスを持たない過去の記事(この機能の導入より前に生成された記事)しかない場合は「前回掲載時のステータスは不明」として扱う(requirements.md#掲載実績の追跡-13)
...
- 関連するビジネスルール: requirements.md#機能要件-6、requirements.md#掲載実績の追跡-12〜13
...
- 各`observation`: `genre`が定義済みジャンルのいずれかであること、`title`が空文字でないこと、`strength`が有限の数値であること、`method`が`fixed-list`または`websearch`であること、`currentRegions`が文字列の配列であること、`strengthJapan`・`strengthOverseas`が数値またはnullであること
...
app/trend-digest/lib/watchlistTypes.ts (既存: CriteriaにHistoryCriteriaを追加)
app/trend-digest/lib/historyTypes.ts (新規: TrendStatus/Observation/ObservationLog/CandidateHistory/StatusJudgementの型定義)
...
app/trend-digest/lib/judgeStatus.ts (新規: 系列からステータス・継続日数・掲載可否を判定する純粋関数)
...
- 地域情報は情報源から判定できた範囲のみを記録し、推測で埋めない(requirements.md#地域情報-9)。判定できない項目を「不明」として保持することで、誤った地域情報が記事に表示されることを防ぐ
