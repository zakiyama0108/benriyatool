...
### 履歴データの保持期間
- [12] 履歴データは削除・自動アーカイブせず、蓄積し続ける(STABLE判定(90日以上)や将来的な傾向分析に過去データが必要なため)
...
- 判定したステータス・地域情報は、content-selectionの掲載可否判定と、[article-detail](../article-detail/requirements.md)でのステータスバッジ・継続期間・地域表示に使われる
