...
- 手順:
  ...
  6. 候補の報告回数が2回目以降(続報)の場合は、前回掲載時の本文・ステータスをプロンプトにあわせて渡し、前回から何が変わったか(段階の進行・継続期間の伸び・言及の広がり)を軸に本文を書く。前回の本文の表現をなぞらず、かつ前回の記事を読んでいない読者にも通じる最小限の説明を含める(requirements.md#続報の執筆-7〜8)
  7. 中長期ステータス・継続日数・報告回数・地域は渡された値をそのまま扱い、本文でそれと異なる段階・期間・地域を書かない(requirements.md#エージェントの逸脱防止-6)
...
- 適用するガードレール文言(weekly-publishの実行指示に必ず含める。requirements.md#エージェントの逸脱防止-5の具体化):
  > この記事で扱ってよい話題は、content-selectionの採用基準に基づき選定された候補のみである。選定候補に含まれない話題を新たに追加してはならない。本文は独自の視点で再構成した解説とし、原文の構成・表現の順序をそのままなぞってはならない。原文の詳細な数値・結論を網羅的に転記してはならない。渡された中長期ステータス・継続日数・報告回数・地域は事実として扱い、それと異なる段階・期間・地域を本文に書いてはならない。続報の場合は、前回掲載時の本文をそのまま言い換えただけの本文を書いてはならない。
