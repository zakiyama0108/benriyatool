...
- [5] その回に収集した全候補の観測ログ([trend-history/requirements.md](../trend-history/requirements.md))も、記事と同じ実行の中でリポジトリに追加する。記事を公開しない回でも観測ログだけは必ず追加する(観測ログが欠けた回があると、以後の継続日数・ステータス判定が実態とずれるため)
...
### 掲載件数の保証
- [1] [content-selection/requirements.md](../content-selection/requirements.md)の基準を満たし、かつ中長期トレンドとして掲載可能と判定された候補があるジャンルだけを掲載し、候補がないジャンルは掲載しない(架空の話題を作らない)。その回に対象ジャンルの掲載可能な候補が1件もない場合のみ、その回の記事公開をスキップする(スキップした回も観測ログは追加する。上記「実行」[5])
...
### 自動マージの範囲
- [3] 完全自動マージの対象はこの週次記事生成PR(記事を公開しない回の観測ログのみのPRを含む)に限る。情報源や採用基準そのものの変更([source-review/requirements.md](../source-review/requirements.md))は対象外とし、別途人間の承認を必須とする(根拠: 週次記事1件の影響範囲は小さいが、基準・情報源の変更は以後の全記事の方向性を左右する重い変更のため)
