...
- [5] その回に収集した全観測項目の観測ログ([trend-history/requirements.md](../trend-history/requirements.md))も、記事と同じ実行の中でリポジトリに追加する。記事を公開しない回でも観測ログだけは必ず追加する(観測ログが欠けた回があると、以後の継続日数・ラベル判定が実態とずれるため)。これは情報源から項目を取得できなかったことによる正常なスキップの回だけでなく、**候補の生成に失敗して実行が失敗した回にも適用する**(生成失敗はリトライしても起こりうる想定内の失敗であり、そのたびに観測が欠けると判定の土台が崩れるため)
