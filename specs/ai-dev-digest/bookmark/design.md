...
- ページ下部のログイン導線: ログインボタンの文言を「ログイン」に変更する(requirements.md#画面共通のログイン導線-16)。ログイン中は、ログイン状態表示の近くに付箋一覧ページへのリンクを追加する【推測: 具体的な文言・配置。要件はリンクの存在自体を明記していないが、requirements.md#付箋した記事一覧-10「新しい画面で確認できる」を満たすための入口が画面のどこかに必要なため】
