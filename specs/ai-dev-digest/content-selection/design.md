...
- 手順:
  ...
  4. 基準を満たす候補・基準を満たさない候補を合わせても`dailyTopicCount.min`件に満たない場合は、件数を無理に満たすために存在しない話題を作らない。その日は記事を生成せず、「候補不足によりスキップ」として記録する【推測】(要件に想定のない極端なケースへの安全策。件数確保を優先して架空の話題を作ることは、content-generation/requirements.md#エージェントの逸脱防止の趣旨に反するため明示的に禁止する)
...
- 候補が最低件数(3件)に満たない場合は記事生成をスキップする(処理フロー参照)。これは[daily-publish](../daily-publish/requirements.md)側の実行が失敗したことにはせず、「その日は正常に0件と判断した」結果として扱う(CI失敗やPR作成失敗とは区別する。daily-publish/design.md#エラーハンドリング参照)
