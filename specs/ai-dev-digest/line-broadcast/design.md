...
- テストからの注入(trend-digestのみ): trend-digestの配信CLI(`scripts/trend-digest/broadcast-line.ts`の`broadcastArticle(articlePath, accessToken)`)に、待機の設定(`pollIntervalMs`・`timeoutMs`・`sleep`)を差し替えるための任意引数を1つ足し、そのまま`waitForPageAvailable`へ渡す。既定値は本番用(15秒・10分)のままとし、テストだけが短い値やフェイクの`sleep`を渡せるようにする(これがないと、時間切れの挙動を検証するテストが実時間で10分待つことになり書けない。trend-digestのtasks.md Task C 🔴が追加する時間切れケースがこれに当たる)。ai-dev-digest・news-digestの配信CLIは`main()`のみでテスト対象の関数をexportしておらず(tasks.md Task CもTDD対象外)、注入口は不要
