...
- [3] 選定された個々の候補について翻訳・要約の生成([content-generation/requirements.md](../content-generation/requirements.md))が失敗した場合、その候補だけを除外し、生成に成功した残りの候補でその日の記事を公開する(1件の生成失敗でその日全体を落とさない)。生成に成功した候補が1件以上あればその件数で公開し、選定された全候補の生成が失敗した日のみ、その日はスキップ扱いとする(根拠: [1]の「実在する候補が1件以上ある限り公開する」趣旨を、候補は実在するが生成が失敗した場合にも適用する。2026-08、1候補の非JSON応答で丸一日が欠損した事象を受けた対応)。生成失敗を「一時的な失敗」と「利用枠の枯渇等の恒久的な失敗」に区別した扱い、および全候補失敗時の運営者の把握手段は設計で確定する
