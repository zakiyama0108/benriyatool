...
- Task 6: 生成失敗時のふるまいの確認(仕様: requirements.md#掲載件数の保証-3、design.md「1日分の記事を生成する処理」手順4-5・「エラーハンドリング」)(リトライ・除外・枯渇打ち切りのロジック自体はcontent-generation/tasks.md Task 8-9のテストで担保する。本タスクは実行フロー全体での配線確認)
  - 一部候補の生成が失敗するケースで、失敗候補が除外され残りの候補でPRが作成されることを確認する(1件の失敗で丸一日が落ちないこと)
  - 全候補の生成が失敗するケース・利用枠枯渇のケースで、`generate-content.ts`が非ゼロ終了し、ブランチ・PRが作られず実行が失敗(赤)として残ることを確認する(候補不足の正常スキップ(緑)と区別されること)
