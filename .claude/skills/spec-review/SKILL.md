...
- 指摘なし(🟢のみ) → [/pr](../pr/SKILL.md)で仕様承認PRを作成し、ユーザーの承認を得る(この場合、この時点の結論はrequirements.md/design.md/tasks.mdに反映済みのため、ここでcompactや新しいセッションへの切り替えを行っても支障はない。切り替える場合は、新しいセッションの名称(前のセッションと同じ<機能名>に次工程名を()で添えた形。例: `<機能名>(仕様承認PR)`)と、次のセッションにそのまま貼り付けられるプロンプトをコードブロックで提示してから終える(例: `/pr を実行してください。仕様承認PR(PR #<番号>)を仕上げてください。`)。指摘ありの場合は指摘内容がまだファイル化されていないため、そのまま/resolveに進むこと)
