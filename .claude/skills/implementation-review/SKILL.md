...
- 指摘なし(🟢のみ) → [/pr](../pr/SKILL.md)で実装PRを作成する(この場合、コードは実装済み・指摘なしの結論も出ているため、ここでcompactや新しいセッションへの切り替えを行っても支障はない。切り替える場合は、新しいセッションの名称(前のセッションと同じ<機能名>に次工程名を()で添えた形。例: `<機能名>(実装PR)`)と、次のセッションにそのまま貼り付けられるプロンプトをコードブロックで提示してから終える(例: `/pr を実行してください。実装PRを作成してください。対象ブランチは feature/<機能名>です。`)。指摘ありの場合は指摘内容がまだファイル化されていないため、そのまま/resolveに進むこと)
