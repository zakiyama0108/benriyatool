...
# UI統合タスク(v0生成コードの取り込み)

[/design](../design/SKILL.md#uiデザインの確定step0画面を伴う機能のみ)のStep0でv0によるUIデザインを確定した機能では、tasks.mdの最初のタスクが「v0生成コードの取り込み・整形」になっている。このタスクはTDDサイクル(🔴🟢🔵)の対象外として扱う(ディレクトリ配置・命名・shadcn/ui最適化・Tailwindクラス整理といった構造上の整形であり、テストで検証すべきロジックがないため)。

- メインスレッドが直接行うか、ui-integratorエージェント(`.claude/agents/ui-integrator.md`)に委譲する(委譲基準はimplementerエージェントと同じ。並行開発時など)
- 完了の基準: 既存のディレクトリ構成・命名規則・shadcn/uiの使い方・テーマ(Light/Dark)対応に沿っていて、React Hook Form/Zod/Server Actions/APIなど実装が必要な箇所が分かりやすく整理されていること(実際の配線は後続タスクで行う)
- このタスクが終わったら、後続タスクは通常のTDDサイクルで進める
...
この時点の成果物(実装・テスト)はコミット済みのため、ここでcompactや新しいセッションへの切り替えを行っても支障はない。切り替える場合は、新しいセッションの名称(前のセッションと同じ<機能名>に次工程名を()で添えた形。例: `<機能名>(コードレビュー)`)と、次のセッションにそのまま貼り付けられるプロンプトをコードブロックで提示してから終える(例: `/implementation-review を実行してください。対象ブランチは feature/<機能名>です。`)。