...
- エージェントの報告をそのままユーザーに提示し、問題があれば次のステップ(/fix)を案内する。報告を要約して提示する場合(autopilot等)も、Step2の本番URL提示(目視の確認項目がある場合)は必ず残す
- エージェントはRead/Grep/Glob/Bashのみを持ち、ファイル編集権限を持たない。Step4(進捗状況の確認・是正)はエージェントの報告を受け取った**メインスレッドが**行う
...
# Step4 進捗状況の確認(メインスレッドが行う。architecture.mdを持つアプリのみ)

機能マップの状態列「リリース済み」への更新は、実装PR・修正PRのコミットに含める運用([/pr](../pr/SKILL.md)の[references/impl-pr.md](../pr/references/impl-pr.md)・[architecture-workflow](../architecture-workflow/SKILL.md)の「更新タイミング」)。**状態更新だけのための後追いPR(旧`feature/<アプリ名>-progress-update`)は作らない。**

このStepでメインスレッドが行うのは確認だけ:

- Step2のスモークチェックで問題なし → 機能マップは実装PRで既に「リリース済み」になっているはず。`git show origin/main:specs/<アプリ名>/architecture.md`で該当spec行を確認し、まだ「実装中」等になっていたら実装PRでの更新漏れなので、この場で是正の小PRを出す(この場合のみ後追いPRを許容)
- Step2で本番に問題を発見 → メインスレッドが機能マップの該当spec行を「実装中」に戻す是正PRを出す(featureブランチを切ってコミット・push・PR作成)。あわせて[/fix](../fix/SKILL.md)で修正フローへ

いずれの場合も、確認・是正後の機能マップ全体(表)をこの完了報告にそのまま貼って提示する(そのアプリの他specの状態も含め、1画面で全体進捗が分かるようにする)。architecture.mdを持たないアプリ(単一spec)ではこのStepは不要。
