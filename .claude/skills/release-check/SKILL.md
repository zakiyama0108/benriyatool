...
- 報告の最後には、確認した本番URL(装飾なしの単独行。トップではなく変更が反映された対象ページのURL)を明記し、実際の画面をユーザー自身の目でも確認するよう案内する(自動チェックはHTTPステータス・HTML文言の機械的な確認に留まり、見た目・操作感まではカバーできないため、UIに見える変更かどうかによらず毎回行う)
