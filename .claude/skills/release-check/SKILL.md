...
いずれの場合も、この機能に関する一連の作業は完了している(mainにマージ・ブランチも掃除済み)ため、ここでcompactや新しいセッションへの切り替えを行っても支障はない。/data-checkに続ける場合は、次のセッションにそのまま貼り付けられるプロンプトをコードブロックで提示してから終える。プロンプトの1行目に新しいセッションの名称(前のセッションと同じ<アプリ名>に次工程名を()で添えた形)を`(セッション名: <アプリ名>(データ確認))`のように含める(例: `(セッション名: <アプリ名>(データ確認))` の次行に `/data-check を実行してください。対象は <アプリ名>の<テーブル名>です。`)。