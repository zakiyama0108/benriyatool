...
context=$(cat <<'EOF'
【ワークフロー・ルーティング】この依頼が開発作業(機能追加・既存機能の修正・仕様作成・レビュー・指摘対応・PR作成・リリース確認)に該当する場合、必ず該当する工程Skill(.claude/skills/README.md参照)をSkillツールで起動してから作業を始めること。
...
- 定期作業(法令確認・依存更新・データ確認・仕様棚卸し・振り返り)はユーザーが `/law-revision-check` のように明示起動する。自律起動はできないので、ここでは扱わない
