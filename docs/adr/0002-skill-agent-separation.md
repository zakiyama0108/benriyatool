...
| UIデザイン確定(Step0・画面を伴う機能のみ) | design | メインスレッド(ヒアリング・候補承認・Stitch MCPツールでの画面生成)+任意で**ui-designer**へ候補生成(デザイントークン・Stitch生成用プロンプト作成)を委譲 | 対象外(autopilotでは扱わない。画面を伴う機能でUIデザイン確定が必要な場合は通常フローで行う) | ヒアリング・承認判断・実際のStitch呼び出しはユーザーとの対話そのものなのでメインスレッドに残す。候補生成は別コンテキストの新鮮な目で複数案を出す価値があり、書き込みツールを持たせない構造で「実装しない」を強制できる(詳細は[下記](#uiデザインの委譲ui-designerエージェント2026-07追記2026-08更新)参照) |
...
現在の割り当て: impl-pr-reviewer / release-checker = haiku、implementer / law-revision-checker / ui-checker / **designer / resolver** / **ui-designer** = sonnet、spec-reviewer / code-reviewer = inherit。
...
7. **ui-designer**(導入済み。2026-07、当初はv0を使ったUI開発を取り入れるにあたりUIデザイン設計を専門化(コード統合を担う**ui-integrator**と対で導入)。2026-08にStitchへの移行に伴いui-integratorは廃止し、ui-designer単独に整理。詳細は[下記](#uiデザインの委譲ui-designerエージェント2026-07追記2026-08更新))
...
### UIデザインの委譲(ui-designerエージェント、2026-07追記、2026-08更新)

v0(https://v0.app等のAIコード生成ツール)を使ったUI開発を取り入れるにあたり、「UIデザインの設計」と「生成コードの既存プロジェクトへの統合」を2つのAgentに分離した(当初は**ui-designer**と**ui-integrator**)。単一責任の原則(SRP)に基づき、判断の性質が異なる2工程だったためである:

- **ui-designer**: 要件・UX・情報設計(IA)・レイアウト・状態・レスポンシブ・アクセシビリティといった「どんな画面にすべきか」の設計判断。実装を行わない構造(書き込みツールなし)にすることで「設計と実装を混ぜない」を強制する
- **ui-integrator**(2026-08廃止): v0生成コードのディレクトリ配置・命名・shadcn/ui最適化・Tailwind整理・テーマ対応といった「既存プロジェクトの規約に合わせる」整形作業。ロジック配線・テストは対象外とし、後続の通常タスク(implementer/メインスレッド)に委ねる想定だった

**なぜメインスレッドにヒアリング・承認判断を残すか**: 本ADRの技術特性(サブエージェントは途中でユーザーに質問できない)により、「どんな雰囲気が好みか」「候補のうちどれを選ぶか」というユーザーとの往復はメインスレッドの仕事のまま残す。ui-designerは「ヒアリング結果を受けて複数候補(デザイントークン・Stitch生成用プロンプト)を作る」という完結した生成作業だけを担当する。この構造は/design・/resolveで「対話が本体の工程はメインスレッドに残す」としてきた判定基準と同じである
...
**配置スコープ**: プロジェクトスコープ(`.claude/agents/`)に置く。ui-designerが担う設計知識(UX原則・デザイン生成ツールへのプロンプトの書き方)自体は他プロジェクトでも通用する汎用的なものだが、本ADRの管理体制(全Agentをこのリポジトリでモデル選定・導入順とあわせて一元管理する)と一致させることを優先した。他プロジェクトで使う場合はファイルをコピーする

**モデル**: 「拘束された作業」(要件という具体的な入力がある)に当たるためsonnet。下流には(承認判断という形の)メインスレッドの判断が品質ゲートとして控えている

**2026-08: v0からGoogle Stitchへの移行とui-integratorの廃止**: v0は「プロンプト→WebUI上でコード生成→ZIPダウンロード→リポジトリへ取り込み」という、コード生成物をエージェント外のWebUIとの手作業往復で取り込む方式だった。これに対しGoogle Stitchは`mcp__stitch__*`のMCPツール群(`create_project`/`generate_screen_from_text`/`edit_screens`/`generate_variants`/`create_design_system`等)を提供しており、メインスレッドが直接呼び出して画面生成・デザインシステム確定まで完結できる。加えてStitchが出力するのはコードではなく画面デザイン(スクリーンショット)とデザイントークン(配色・フォント・角丸等)であり、v0のような「生成コードをリポジトリの規約に整形して取り込む」工程がそもそも発生しない。そのため、この整形作業を専任していた**ui-integrator**は担当する仕事自体がなくなり廃止した。実装は、design.mdに書いたStitchの参照(プロジェクトID・デザインシステム・採用スクリーン)を見ながら、通常のTDDタスク(implementer/メインスレッド)としてコーディングする(v0のようなコード取り込みタスクを`tasks.md`の先頭に置く運用も合わせて廃止)。ui-designerは実装を持たない構造上の理由は変わらず存続するが、成果物を「v0向けプロンプト」から「Stitchのデザイントークン案・生成プロンプト」に変更した
