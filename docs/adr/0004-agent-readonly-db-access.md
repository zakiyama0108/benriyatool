...
- **ロール作成のみ**[ADR-0003](0003-db-schema-migration-ci.md)の`supabase/migrations/`には含めない。パスワードが伴い、git管理下のファイルに残せないため、初回1回のみSupabaseダッシュボードのSQLエディタで手動実行する(下記「作業手順」参照)。**GRANT・RLSポリシー追加はパスワードを含まないため、新しいテーブルを作るマイグレーションファイルにそのまま含めてよい**(`ai_dev_digest_feedback`のマイグレーションが実例。2026-08第3次改定: 当初は「新しいアプリのテーブルを追加する際は、上記2.と同じ2行をそのテーブルにも追加する」という運用のみ文書化されていたが、`ai_dev_digest_bookmarks`のマイグレーションでこの手順そのものが漏れ、data-check実行時までSELECT権限がないことに気づけなかった。原則として**個人データを含むテーブルであっても新規テーブルには必ずこの2行を追加する**ことを明記し、[design skill](../../.claude/skills/design/SKILL.md)のデータベース設計項目からも参照させることで、設計段階でのチェック漏れを防ぐ)
